...
A1_field = data_laser[:, 2]
A2_field = data_laser[:, 5]
A_field = A1_field + A2_field
...
def my_fourier_transform(signal, time_dt, time, ww_omega):
    ft_signal = np.zeros(len(ww_omega)).astype(np.complex128)
    for idx in range(len(ww_omega)):
        ft_signal[idx] = \
            (signal*np.exp(1j*ww_omega[idx]*time)).sum()*time_dt
    return ft_signal
...
ax.plot(time_fs, A1_field, 'r', linestyle='-')
ax.margins(x=0)
ax.set_xlabel('Time (fs)', fontsize=15)
ax.set_ylabel(r'$A_{1}(t)$ (a.u.)', fontsize=12, color="r")
ax.xaxis.set_visible(False)
ax.get_yaxis().set_label_coords(-0.1, 0.5)
ax.tick_params(axis='both', which='major', labelsize=13)
# Second plot 
ax = fig.add_subplot(gs[1])
ax.plot(time_fs, A2_field, 'b', linestyle='-')
ax.margins(x=0)
ax.set_xlabel('Time (fs)', fontsize=15)
ax.set_ylabel(r'$A_{2}(t)$ (a.u.)', fontsize=12, color="b")
ax.xaxis.set_visible(False)
ax.get_yaxis().set_label_coords(-0.1, 0.5)
ax.tick_params(axis='both', which='major', labelsize=13)
# Third plot
ax = fig.add_subplot(gs[2])
ax.plot(time_fs, A_field, 'm', linestyle='-')
ax.margins(x=0)
ax.set_xlabel('Time (fs)', fontsize=15)
ax.set_ylabel(r'$A_{1}(t)+A_{2}(t)$ (a.u.)', fontsize=11, color="m")
ax.get_yaxis().set_label_coords(-0.1, 0.5)
ax.tick_params(axis='both', which='major', labelsize=13)
plt.savefig('graphene_A_field.png', bbox_inches="tight", dpi=200)
plt.show()
