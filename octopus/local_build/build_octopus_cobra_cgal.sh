...
if [[ $compiler == intel ]]; then
  ...
  if [[ $mpi == impi ]]; then
    module load gcc/10 impi/2021.2 mkl/2021.2
    export CC=mpicc
    export FC=mpif90
    export CXX=mpicxx
    ...
    module load gcc/10 openmpi/4 mkl/2021.2
    export CC=mpicc
    export FC=mpif90
    export CXX=mpicxx
...
module load gsl hdf5-serial netcdf-serial libxc metis parmetis cgal boost/1.74
if [[ $compiler == intel ]]; then
...
if [[ "$2" == "nodep" ]]; then
  make FCFLAGS="$FCFLAGS" NODEP=1 -j20 && make -j20 NODEP=1 install || exit 1
else
  make FCFLAGS="$FCFLAGS" -j20 && make install || exit 1
fi
