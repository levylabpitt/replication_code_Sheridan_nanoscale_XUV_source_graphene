...
if test x"$FCFLAGS_FFTW" = x; then
  ...
  if test x$with_fftw_prefix != x; then
    ...
    if test x"$acx_fftw_ok" = xyes; then
      AC_MSG_ERROR([--with-fftw-prefix was given, but it is possible that FFTW is being provided by another library (e.g., MKL). Make sure no other library provides FFTW or that you are not specifying the FFTW libraries through other environment variables (e.g., LIBS). Also make sure only the header location is specified in FCFLAGS_FFTW.])
    fi
