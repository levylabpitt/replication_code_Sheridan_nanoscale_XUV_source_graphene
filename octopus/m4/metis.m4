...
#include <metis.h>
#if IDXTYPEWIDTH == 32
  #error index type width is 32
#endif
],[ ])], [acx_metis_idxtype64=yes], [acx_metis_idxtype64=no])
...
AC_MSG_CHECKING([METIS index width is 64 bit])
AC_MSG_RESULT([$acx_metis_idxtype64])
if test x"$acx_metis_idxtype64" = xyes; then
  AC_DEFINE(METIS_IDXTYPEWIDTH,64,[This defines the width of the index type in METIS.])
else
  AC_DEFINE(METIS_IDXTYPEWIDTH,32,[This defines the width of the index type in METIS.])
fi
