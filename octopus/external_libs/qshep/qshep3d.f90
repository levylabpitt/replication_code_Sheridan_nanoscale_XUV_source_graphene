...
!
  ...
  integer(int64) jp1
  integer(int64) k
  integer(int64) lcell(nr,nr,nr)
  integer(int64) lmax
  integer(int64) lnext(n)
  integer(int64) lnp
  integer(int64) neq
  integer(int64) nn
  integer(int64) nnq
  ...
  integer(int64) nnw
  integer(int64) np
  integer(int64) npts(40)
  integer(int64) nq
  integer(int64) nqwmax
  integer(int64) nw
  real ( kind = 8 ) rmax
  real ( kind = 8 ) rq
  real ( kind = 8 ) rs
  ...
  real ( kind = 8 ) rsold
  real ( kind = 8 ) rsq(n)
  real ( kind = 8 ), parameter :: rtol = 1.0E-05
  real ( kind = 8 ) rws
  real ( kind = 8 ) s
...
!  centered at P.
!
  imin = int((xp-xmin-rmax)/dx, int64) + 1
  imin = max ( imin, 1 )

  imax = int((xp-xmin+rmax)/dx, int64) + 1
  imax = min ( imax, nr )

  jmin = int((yp-ymin-rmax)/dy, int64) + 1
  jmin = max ( jmin, 1 )

  jmax = int((yp-ymin+rmax)/dy, int64) + 1
  jmax = min ( jmax, nr )

  kmin = int((zp-zmin-rmax)/dz, int64) + 1
  kmin = max ( kmin, 1 )

  kmax = int((zp-zmin+rmax)/dz, int64) + 1
  kmax = min ( kmax, nr )
!
...
!
  ...
  dely = yp - xyzmin(2)
  delz = zp - xyzmin(3)
  i0 = int(delx/dx, int64) + 1
  if ( i0 < 1 ) i0 = 1
  if ( nr < i0 ) then
    i0 = nr
  end if

  j0 = int ( dely / dy, int64 ) + 1
  if ( j0 < 1 ) j0 = 1
  if ( nr < j0 ) then
    j0 = nr
  end if

  k0 = int(delz/dz, int64) + 1
  if ( k0 < 1 ) k0 = 1
  if ( nr < k0 ) then
    k0 = nr
  end if
...
!
  ...
  rsmin = rsq
  r = sqrt(rsmin)
  imin = int((delx-r)/dx, int64) + 1
  if ( imin < 1 ) imin = 1
  imax = int((delx+r)/dx, int64) + 1
  if ( nr < imax ) imax = nr
  jmin = int((dely-r)/dy, int64) + 1
  if ( jmin < 1 ) jmin = 1
  jmax = int((dely+r)/dy, int64) + 1
  if ( nr < jmax ) jmax = nr
  kmin = int((delz-r)/dz, int64) + 1
  if ( kmin < 1 ) kmin = 1
  kmax = int((delz+r)/dz, int64) + 1
  if ( nr < kmax ) kmax = nr
  first = .false.
  go to 4
!
...
!
  ...
  do ll = 1, nn
    lb = np1 - ll
    i = int((x(lb)-xmn)/delx, int64) + 1
    if ( nnr < i ) i = nnr
    j = int((y(lb)-ymn)/dely, int64) + 1
    if ( nnr < j ) j = nnr
    k = int((z(lb)-zmn)/delz, int64) + 1
    if ( nnr < k ) k = nnr
