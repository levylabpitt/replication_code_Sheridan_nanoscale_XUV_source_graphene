...
subroutine X(output_me_ks_multipoles)(fname, namespace, space, st, mesh, ll, mm, ik)
  ...
  SAFE_ALLOCATE(multipole(1:mesh%np))
  ...
  do ist = 1, st%nst
    ...
    do jst = 1, st%nst
      ...
      psij(1:mesh%np, 1) = psij(1:mesh%np, 1)*multipole(1:mesh%np)

      multip_element = X(mf_dotp)(mesh, st%d%dim, psii, psij)
