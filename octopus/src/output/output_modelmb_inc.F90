...
!
subroutine X(output_modelmb) (outp, namespace, space, dir, mesh, st, ions)
  ...
  if (st%parallel_in_states) then
    call messages_not_implemented("Model MB output parallel in states", namespace=namespace)
  end if
...
!TODO : check if there is another interface for get_states to avoid trivial slice of wf
    ...
    if (outp%what(OPTION__OUTPUT__MMB_DEN) .and. symmetries_satisfied) then
      call X(modelmb_density_matrix_write)(space, mesh, st, wf, mm, denmat, namespace)
    end if
