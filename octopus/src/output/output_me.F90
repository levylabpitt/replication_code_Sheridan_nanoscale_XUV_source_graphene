...
contains
  ...
  subroutine output_me(this, namespace, space, dir, st, gr, ions, hm)
    ...
    if (this%what(OPTION__OUTPUTMATRIXELEMENTS__ANG_MOMENTUM)) then
      ...
      call output_me_out_ang_momentum(fname, st, gr, namespace, space, hm%kpoints)
    end if
  ...
  ! ---------------------------------------------------------
  subroutine output_me_out_ang_momentum(fname, st, gr, namespace, space, kpoints)
    character(len=*),    intent(in)    :: fname
    type(states_elec_t), intent(inout) :: st
    type(grid_t),        intent(in)    :: gr
    type(namespace_t),   intent(in)    :: namespace
    ...
    FLOAT              :: angular(3), lsquare, kpoint(space%dim)
    ...
    if (states_are_real(st)) then
      call dstates_elec_angular_momentum(st, space, gr, ang, ang2)
    else
      call zstates_elec_angular_momentum(st, space, gr, ang, ang2)
    end if
    ...
    if (st%d%kpt%parallel) then
      ...
      do idir = 1, 3
        ...
        call st%d%kpt%mpi_grp%allgatherv(lang, st%nst*kn, MPI_FLOAT, &
