...
!! Copyright (C) 2020-2021 M. Oliveira, K. Lively, A. Obzhirov, I. Albar
!!
...
!!$    call loct_progress_bar(-1, l_end-l_start)
  ...
  subroutine local_read_from_block(domain, space, ions, blk, row, namespace)
    ...
    integer :: ic, ia
    FLOAT   :: radius, center(space%dim), half_length(space%dim)
    ...
    case (MINIMUM)
      ...
      do ia = 1, ions%natoms
        if (loct_isinstringlist(ia, domain%clist)) then
          call minimum_box%add_box(box_sphere_t(space%dim, ions%pos(:, ia), radius, namespace))
        end if
      end do
    ...
    case (SPHERE)
      ...
      domain%box => box_sphere_t(space%dim, center, radius, namespace)
    ...
    case (CYLINDER)
      ...
      call parse_block_float(blk, row, 3, half_length(1), unit = units_inp%length)
      do ic = 1, space%dim
        call parse_block_float(blk, row, 3 + ic, center(ic), unit = units_inp%length)
      end do

      domain%box => box_cylinder_t(space%dim, center, ions%latt%rlattice, radius, half_length(1)*M_TWO, namespace)

    case (PARALLELEPIPED)
      do ic = 1, space%dim
        call parse_block_float(blk, row, 1 + ic, half_length(ic), unit = units_inp%length)
      end do
      do ic = 1, space%dim
        call parse_block_float(blk, row, 1 + space%dim + ic, center(ic), unit = units_inp%length)
      end do

      domain%box => box_parallelepiped_t(space%dim, center, ions%latt%rlattice, half_length*M_TWO, namespace)
  ...
  subroutine create_basins(namespace, space, mesh, ions, ff, basins)
    ...
    call basins_init(basins, namespace, mesh)
    call basins_analyze(basins, namespace, mesh, ff2(:,1), ff2, bader_threshold)
    ...
    if (use_atomic_radii) then
      SAFE_ALLOCATE(ion_map(1:ions%natoms))
      ion_map = 0
      do ia = 1, ions%natoms
  ...
  subroutine bader_domain_create_mask(domain, basins, mesh, ions)
    ...
    do ia = 1, ions%natoms
      ...
      ib = ib + 1
      ix = mesh_nearest_point(mesh, ions%pos(:, ia), dmin, rankmin)
  ...
  subroutine local_ions_mask(mesh_mask, ions, mesh, ions_mask)
    ...
    SAFE_ALLOCATE(mask_tmp(1:ions%natoms))
    mask_tmp = 0
