...
contains
  ...
  subroutine unfold_setup()
    ...
    if (mpi_grp_is_root(mpi_world)) then
      ...
      do ik = 1, path_kpoints_grid%npoints
        gvec(:) = nint(path_kpoints_grid%red_point(:, ik) + M_HALF * CNST(1e-7))
        ...
        write(file_gvec,'(*(i3))') (gvec(idir), idir = 1, sys%space%dim)
      end do
      write(file_kpts,'(a)') '%'
      call io_close(file_gvec)
  ...
  subroutine wfs_extract_spec_fn(space, st, gr, zcube, cf)
    ...
    FLOAT :: vec_pc(space%dim), vec_sc(space%dim)
    ...
    do ik = st%d%kpt%start, st%d%kpt%end
      ...
      case (3)
        do ig = 1, shell%ngvectors
          call kpoints_to_absolute(sys%kpoints%latt, TOFLOAT(shell%red_gvec(:,ig)), vec_sc(:))
          do ix = gmin, gmax
            do iy = gmin, gmax
              do iz = gmin, gmax
