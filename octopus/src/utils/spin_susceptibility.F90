...
program spin_susceptibility
  ...
  if (parse_is_defined(global_namespace, 'TransientMagnetizationReference')) then
    ...
    SAFE_ALLOCATE(m_cart_ref(1:time_steps_ref, 1:num_col_cart))
    call io_skip_header(ref_file)
  ...
  do iq = 1, kick%nqvec
    ...
    SAFE_ALLOCATE(ftreal(1:energy_steps, 1:num_col))
    SAFE_ALLOCATE(ftimag(1:energy_steps, 1:num_col))
    ...
    write(header, '(9a18)') '#        energy', 'Re[\chi_{+-}(q)]', 'Im[\chi_{+-}(q)]', &
      'Re[\chi_{-+}(-q)]', 'Im[\chi_{-+}(-q)]', &
