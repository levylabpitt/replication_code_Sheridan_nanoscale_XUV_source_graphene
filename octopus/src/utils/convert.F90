...
contains
  ...
  subroutine convert_transform(mesh, namespace, space, ions, mc, kpoints, basename, in_folder, c_start, c_end, c_step, outp, &
    ...
    call messages_print_var_value('ConvertEnergyMin', min_energy, unit = units_out%energy, iunit=wd_info)
    ...
    call messages_print_var_value('ConvertReadSize', chunk_size, iunit=wd_info)
    ...
    call messages_print_var_value('ConvertEnergyMax', max_energy, unit = units_out%energy, iunit=wd_info)
    ...
    call messages_print_var_option('ConvertFTMethod', ft_method, iunit=wd_info)
    ...
    call messages_print_var_value('ConvertEnergyStep', dw, unit = units_out%energy, iunit=wd_info)
      ...
      do i_time = c_start, c_end, c_step
        ...
        if (mesh%parallel_in_domains .and. i_space == 1) then
          ...
          if (mod(i_space-1, chunk_size) == 0) then
            ...
            call io_binary_read(trim(filename), i4_to_i8(chunk_size), point_tmp(1:chunk_size, t_point), &
              ierr, offset=i4_to_i8(i_space-1))
      ...
      if (mesh%mpi_grp%size == 1) then
        if (mod(i_space, chunk_size) == 0) then
          ...
          do i_energy = e_start, e_end
            ...
            if (i_space == chunk_size) then
              call dwrite_header(trim(filename), mesh%np_global, ierr)
            end if
            call io_binary_write(trim(filename), i4_to_i8(chunk_size), point_tmp(1:chunk_size, i_energy), ierr, &
              nohead = .true.)
          end do
        end if
      end if
    ...
    if (mesh%parallel_in_domains) then
      ...
      if (any(outp%how /= OPTION__OUTPUTFORMAT__BINARY)) then
        do i_energy = e_start, e_end
          write(filename,'(a14,i0.7,a1)')'wd.general/wd.',i_energy,'/'
          call io_binary_read(trim(filename)//'density.obf', i4_to_i8(mesh%np), read_rff, ierr)
          do output_i = lbound(outp%how, 1), ubound(outp%how, 1)
