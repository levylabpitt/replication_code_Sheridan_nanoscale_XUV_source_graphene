...
contains
  ...
  subroutine ps_init(ps, namespace, label, z, user_lmax, user_llocal, ispin, filename)
    ...
    case (PSEUDO_FORMAT_HGH)
      ...
      call ps_guess_atomic_occupations(ps%z, ps%z_val, ps%lmax, 1, ps%conf)
  ...
  subroutine ps_separate(ps)
    ...
    do ii = 1, ps%g%nrval
      ...
      if (ii > 1) then
        ...
        if(arg > CNST(5e-5)) then !To avoid FPE
          vlr(ii)  = -ps%z_val*loct_erf(r/(ps%sigma_erf*sqrt(M_TWO)))/r
        else
          vlr(ii)  = -ps%z_val*M_TWO/(ps%sigma_erf*sqrt(M_TWO*M_PI))
        end if
      end if
  ...
  subroutine ps_filter(ps, filter, gmax)
    ...
    case (PS_FILTER_BSB)
      ...
      ! For the special case l=-1, we use l=0 when doing the filtering, but it is not clear this is correct
      call spline_filter_bessel(ps%vl, max(0, ps%llocal), gmax, alpha, beta_fs, rcut, beta_rs)
      do l = 0, ps%lmax
        if (l == ps%llocal) cycle
        do k = 1, ps%kbc
  ...
  subroutine ps_xml_load(ps, ps_xml)
    ...
    if (ps_xml%kleinman_bylander) then
      ...
      do ll = 0, ps_xml%lmax
        ...
        kbcos = dnrm/(SAFE_TOL(avgv,CNST(1.0e-20)))
        kbnorm = M_ONE/(SAFE_TOL(sqrt(dnrm),CNST(1.0e-20)))
  ...
  subroutine ps_guess_atomic_occupations(zval, valcharge, lmax, ispin, conf)
    ...
    case (71)
      if (val > 23) call fill_s_orbs(val, 2, 5)
      if (val > 17) call fill_p_orbs(val, 6, 5)
    ...
    case (72)
      if (val > 10) call fill_s_orbs(val, 2,  5)
      if (val > 4) call fill_p_orbs(val, 6,  5)
    ...
    case (73)
      if (val > 11) call fill_s_orbs(val, 2, 5)
      if (val > 5) call fill_p_orbs(val, 6, 4)
    ...
    case (79)
      if (val > 25) call fill_p_orbs(val, 6,  5)
      if (val > 14) call fill_f_orbs(val, 14, 4)
    ...
    case (81)
      if (val > 24) call fill_f_orbs(val, 14, 4)
  ...
  contains
    subroutine fill_s_orbs(val, max_occ, nn)
      ...
      conf%p = conf%p + 1
      conf%occ(conf%p,1) = min(val, real(max_occ))
