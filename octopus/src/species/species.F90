...
contains
  ...
  subroutine species_build(spec, namespace, ispin, dim, print_info)
    ...
    case (SPECIES_PSEUDO, SPECIES_PSPIO)
      ...
      if (spec%type == SPECIES_PSPIO) then
        call ps_pspio_init(spec%ps, namespace, spec%label, spec%Z, spec%user_lmax, ispin, spec%filename)
      else
  ...
  subroutine species_real_nl_projector(spec, np, x, r, l, lm, i, uV)
    ...
    PUSH_SUB(species_real_nl_projector)
    ...
    POP_SUB(species_real_nl_projector)
...
! Private procedures
  ...
  subroutine read_from_default_file(iunit, read_data, spec)
    ...
    read(iunit,*) label, spec%filename, zz, lmax, llocal
    ...
    do
      ...
      case (OPTION__SPECIES__LLOC)
        ...
        if (spec%user_llocal < 0) then
          call messages_input_error(namespace, 'Species', &
            "The 'lloc' parameter in species "//trim(spec%label)//" cannot be negative", row=row, column=icol+1)
        end if
      ...
      case (OPTION__SPECIES__HUBBARD_L)
        ...
        if (spec%hubbard_l < 0) then
          call messages_input_error(namespace, 'Species', &
            "The 'hubbard_l' parameter in species "//trim(spec%label)//" cannot be negative", row=row, column=icol+1)
        end if
      ...
      case (OPTION__SPECIES__HUBBARD_J)
        ...
        if (abs(spec%hubbard_j-spec%hubbard_l) /= M_HALF) then
          call messages_input_error(namespace, 'Species', "The 'hubbard_j' parameter in species "// &
            trim(spec%label)//" can only be hubbard_l +/- 1/2", row=row, column=icol+1)
        end if
  ...
  logical function species_is_same_species(spec1, spec2) result(same)
    type(species_t), intent(in) :: spec1, spec2

    PUSH_SUB(species_is_same_species)

    same = .true.
    if(spec1%z /= spec2%z) same = .false.
    if(spec1%z_val /= spec2%z_val) same = .false.
    if(spec1%type /= spec2%type) same = .false.
    if(spec1%mass /= spec2%mass) same = .false.

    !These cases are not treated properly at the moment
    if(spec1%type /= SPECIES_FULL_DELTA .and. spec1%type /= SPECIES_PSEUDO &
      .and. spec1%type /= SPECIES_PSPIO) then
      same = .false.
    end if
