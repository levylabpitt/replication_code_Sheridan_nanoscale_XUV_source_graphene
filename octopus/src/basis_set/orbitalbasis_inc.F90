...
subroutine X(orbitalbasis_build)(this, namespace, ions, mesh, kpt, ndim, skip_s_orb, use_all_orb, verbose)
  ...
  if (accel_is_enabled() .and. ndim == 1) then
    do iorbset = 1, this%norbsets
      os => this%orbsets(iorbset)
...
subroutine X(orbitalbasis_build_empty)(this, mesh, kpt, ndim, nstates, verbose)
  ...
  integer :: ios, iorb, offset, iorbset
  type(orbitalset_t), pointer :: os
  logical :: verbose_
#ifdef R_TCOMPLEX
  integer :: ik
  ...
  if (accel_is_enabled() .and. ndim == 1) then
    do iorbset = 1, this%norbsets
      ...
      os%ldorbs = max(pad_pow2(os%sphere%mesh%np), 1)
      call accel_create_buffer(os%X(buff_orb), ACCEL_MEM_READ_ONLY, R_TYPE_VAL, os%ldorbs*os%norbs)
      ...
      do ik= kpt%start, kpt%end
        call accel_create_buffer(os%buff_eorb(ik), ACCEL_MEM_READ_ONLY, R_TYPE_VAL, os%ldorbs*os%norbs)
      end do
#endif
    end do
  end if
