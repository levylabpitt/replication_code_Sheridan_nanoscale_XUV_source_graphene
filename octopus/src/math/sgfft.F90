...
contains
  ...
  subroutine fft(n1, n2, n3, nd1, nd2, nd3, z, isign, inzee)
    ...
    real(r8), intent(inout) :: z(1:2, 1:nd1*nd2*nd3, 1:2)
    integer, intent(in)    :: isign
    integer, intent(inout) :: inzee

    real(r8), allocatable :: zw(:, :, :)
    real(r8), allocatable :: trig(:, :)
...
!$    iam = omp_get_thread_num()
  ...
  subroutine ctrig(n,trig,after,before,now,isign,ic)
    ...
    integer :: i, j, itt, nh
    real(r8) :: angle, trigc, trigs, twopi
...
1000 continue
  ...
  subroutine fftstp(mm,nfft,m,nn,n,zin,zout,trig,after,now,before,isign)
    ...
    real(r8) :: trig(:,:) !< (2,<=8192)
    real(r8) :: zin(2, mm, m), zout(2, nn, n)

    real(r8) :: rt2i, dp, cp, cm, ci5, cr5, ci6, cr6, am, ap, ci8, cr8
    real(r8) :: r, r1, r2, r3, r4, r5, r6, r7, r8_, r25, r34
    real(r8) :: s, s1, s2, s3, s4, s5, s6, s7, s8, s25, s34
    real(r8) :: bb, bm, dm, bp
    real(r8) :: cr2, ci2, cr3, ci3, cr4, ci4, cr7, ci7
    real(r8) :: cos2, sin2, cos4, sin4
    real(r8) :: ur1, ur2, ur3, ui1, ui2, ui3
    real(r8) :: vr1, vr2, vr3, vi1, vi2, vi3
  ...
  subroutine fftrot(mm,nfft,m,nn,n,zin,zout,trig,after,now,before,isign)
    ...
    integer :: after, before, atn, atb, now
    real(r8) :: trig(:,:) !< (2,<=8192)
    real(r8) :: zin(2, mm, m), zout(2, n, nn)
...
!!!HERE POT MUST BE THE KERNEL (BEWARE THE HALF DIMENSION)
  ...
  subroutine convolxc_off(n1,n2,n3,nd1,nd2,nd3,md1,md2,md3, &
    ...
    real(r8), dimension(nd1,nd2,nd3/nproc), intent(in) :: pot
    real(r8), dimension(md1,md3,md2/nproc), intent(inout) :: zf
    ...
    real(r8), allocatable :: zt(:,:,:)
    !work arrays for MPI
    real(r8), allocatable :: zmpi1(:,:,:,:,:)
    real(r8), allocatable :: zmpi2(:,:,:,:)
    !cache work array
    real(r8), allocatable :: zw(:,:,:)
    !FFT work arrays
    real(r8), allocatable :: cosinarr(:,:)
    real(r8) :: btrig1(2,8192)
    real(r8) :: ftrig1(2,8192)
    integer :: after1(7)
    integer :: now1(7)
    integer :: before1(7)
    real(r8) :: btrig2(2,8192)
    real(r8) :: ftrig2(2,8192)
    integer :: after2(7)
    integer :: now2(7)
    integer :: before2(7)
    real(r8) :: btrig3(2,8192)
    real(r8) :: ftrig3(2,8192)
    integer :: after3(7)
    integer :: now3(7)
    integer :: before3(7)
...
!!! ehartree=0.0_8
  ...
  subroutine multkernel(nd1,nd2,n1,n2,lot,nfft,jS,pot,zw)
    ...
    real(r8), dimension(nd1,nd2), intent(in) :: pot
    real(r8), dimension(2,lot,n2), intent(inout) :: zw
  ...
  subroutine switch_upcorn(nfft,n2,lot,n1,lzt,zt,zw)
    ...
    real(r8) :: zw(2,lot,n2), zt(2,lzt,n1)
  ...
  subroutine mpiswitch_upcorn(j3,nfft,Jp2stb,J2stb,lot,n1,md2,nd3,nproc,zmpi1,zw)
    ...
    real(r8) :: zmpi1(2,n1/2,md2/nproc,nd3/nproc,nproc),zw(2,lot,n1)
  ...
  subroutine halfill_upcorn(md1,md3,lot,nfft,n3,zf,zw)
    ...
    real(r8) :: zw(2,lot,n3/2),zf(md1,md3)
  ...
  subroutine scramble_unpack(i1,j2,lot,nfft,n1,n3,md2,nproc,nd3,zw,zmpi2,cosinarr)
    ...
    real(r8), dimension(2,lot,n3/2), intent(in) :: zw
    real(r8), dimension(2,n3/2), intent(in) :: cosinarr
    real(r8), dimension(2,n1,md2/nproc,nd3), intent(out) :: zmpi2

    integer :: i3,i,ind1,ind2
    real(r8) ::  a,b,c,d,cp,sp,feR,feI,foR,foI,fR,fI
  ...
  subroutine unscramble_pack(i1,j2,lot,nfft,n1,n3,md2,nproc,nd3,zmpi2,zw,cosinarr)
    integer, intent(in) :: i1,j2,lot,nfft,n1,n3,md2,nproc,nd3
    real(r8), dimension(2,lot,n3/2), intent(out) :: zw
    real(r8), dimension(2,n3/2), intent(in) :: cosinarr
    real(r8), dimension(2,n1,md2/nproc,nd3), intent(in) :: zmpi2

    integer :: i3,i,indA,indB
    real(r8) ::  a,b,c,d,cp,sp,re,ie,ro,io,rh,ih
  ...
  subroutine unfill_downcorn(md1,md3,lot,nfft,n3,zw,zf, scal)
    ...
    real(r8), dimension(2,lot,n3/2), intent(in) :: zw
    real(r8), dimension(md1,md3),intent(inout) :: zf
    real(r8), intent(in) :: scal
...
!!
  subroutine kernelfft(n1,n2,n3,nd1,nd2,nd3,nproc,iproc,zf,zr,comm)
    ...
    real(r8), dimension(nd1,n3,nd2/nproc), intent(in) :: zf
    real(r8), dimension(2,nd1,nd2,nd3/nproc), intent(out) :: zr
    integer, intent(in) :: comm
    ...
    !work arrays for MPI
    real(r8), dimension(:,:,:,:,:), allocatable :: zmpi1
    real(r8), dimension(:,:,:,:), allocatable :: zmpi2
    !cache work array
    real(r8), dimension(:,:,:), allocatable :: zw
    !FFT work arrays
    real(r8), dimension(:,:), allocatable :: trig1,trig2,trig3,cosinarr
...
!        end if
  ...
  subroutine switch(nfft,n2,lot,n1,lzt,zt,zw)
    integer :: lot, n2, lzt, n1, j, nfft, i
    real(r8) :: zw(1:2, 1:lot, 1:n2), zt(1:2, 1:lzt, 1:n1)
  ...
  subroutine mpiswitch(j3,nfft,Jp2st,J2st,lot,n1,nd2,nd3,nproc,zmpi1,zw)
    ...
    real(r8) :: zmpi1(2,n1,nd2/nproc,nd3/nproc,nproc),zw(2,lot,n1)
  ...
  subroutine inserthalf(nd1,lot,nfft,n3,zf,zw)
    ...
    real(r8) :: zw(1:2, 1:lot, 1:n3/2), zf(1:nd1, 1:n3)
