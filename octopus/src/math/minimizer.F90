...
module minimizer_oct_m
  ...
  interface loct_minimize
    integer function oct_minimize(method, dim, x, step, line_tol, &
      ...
      real(r8), intent(in)    :: line_tol
      real(r8), intent(in)    :: tolgrad
      ...
      interface
        ...
        subroutine write_iter_info(iter, n, val, maxdr, maxgrad, x)
          ...
          real(r8), intent(in) :: maxdr
          real(r8), intent(in) :: maxgrad
          real(r8), intent(in) :: x(n)
        end subroutine write_iter_info
      end interface
      real(r8), intent(out)   :: minimum
...
contains
  ...
  subroutine minimize_multidim_nlopt(ierr, method, dim, x, step, toldr, maxiter, f, minimum, lb, ub)
    ...
    real(r8), intent(out)   :: minimum
    real(r8), intent(in), optional :: lb(:), ub(:)
  ...
  subroutine minimize_sd(dim, x, step, maxiter, f, write_iter_info, minimum, ierr)
    ...
    integer :: iter
    real(r8), allocatable :: grad(:)
    real(r8) :: step2, maxgrad
  ...
  subroutine minimize_fire(dim, x, step, tolgrad, maxiter, f, write_iter_info, en, ierr, mass, integrator)
    ...
    real(r8), allocatable :: grad_atoms(:)
    real(r8), allocatable :: vel(:)
    real(r8), allocatable :: dr_i(:)
    real(r8), allocatable :: x_new(:)
    real(r8), allocatable :: dr_atoms(:)
    ...
    do while (n_iter <= maxiter)
      ...
      do i_tmp = 0, dim/3 - 1
        grad_atoms(i_tmp+1) = norm2(grad(3*i_tmp+1:3*i_tmp+3))
        dr_atoms(i_tmp+1) =   norm2(dr_i(3*i_tmp+1:3*i_tmp+3))
      end do
