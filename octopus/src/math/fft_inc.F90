...
#ifdef R_TREAL
  call cuda_fft_execute_d2z(fft_array(slot)%cuda_plan_fw, in%mem, out%mem, accel%cuda_stream)
#else
  call cuda_fft_execute_z2z_forward(fft_array(slot)%cuda_plan_fw, in%mem, out%mem, accel%cuda_stream)
#endif
  call fft_operation_count(fft)
  call accel_finish()
...
subroutine X(fft_backward_3d)(fft, in, out, norm)
  ...
  case (FFTLIB_PFFT)
    if (all(fft_array(slot)%fs_n /= 0)) then
      if (in(1,1,1) /= in(1,1,1)) then
        message(1) = "Error: found NaN in input for FFT!"
        call messages_fatal()
      end if
...
#ifdef R_TREAL
  call cuda_fft_execute_z2d(fft_array(slot)%cuda_plan_bw, in%mem, out%mem, accel%cuda_stream)
#else
  call cuda_fft_execute_z2z_backward(fft_array(slot)%cuda_plan_bw, in%mem, out%mem, accel%cuda_stream)
#endif
  call fft_operation_count(fft)
  call accel_finish()
