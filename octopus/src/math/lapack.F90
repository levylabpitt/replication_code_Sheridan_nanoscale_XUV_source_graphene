...
module lapack_oct_m
  ...
  interface
    subroutine dgeev(jobvl, jobvr, n, a, lda, wr, wi, vl, ldvl, vr, ldvr, work, lwork, rwork, info)
      ...
      real(r8),      intent(out)   :: wr, wi, vl, vr !< wr(n), wi(n), vl(ldvl,n), vl(ldvr,n)
    ...
    subroutine zgeev(jobvl, jobvr, n, a, lda, w, vl, ldvl, vr, ldvr, work, lwork, rwork, info)
      ...
      complex(r8),   intent(out)   :: w, vl, vr !< w(n), vl(ldvl,n), vl(ldvr,n)
      real(r8),      intent(out)   :: rwork !< rwork(max(1,2n))
      complex(r8),   intent(out)   :: work  !< work(lwork)
      integer,      intent(out)   :: info
  ...
  interface lapack_gesvx
    subroutine dgesvx(fact, trans, n, nrhs, a, lda, af, ldaf, ipiv, equed, r, &
      ...
      real(r8),     intent(inout) :: a, af, r, c, b      !< a(lda,n), af(ldaf,n), r(n), c(n), b(ldb,nrhs)
      integer,      intent(inout) :: ipiv                !< ipiv(n)
      real(r8),     intent(out)   :: x, ferr, berr, work !< x(ldx,nrhs), ferr(nrhs), berr(nrhs), work(4*n)
      real(r8),     intent(out)   :: rcond
    ...
    subroutine zgesvx (fact, trans, n, nrhs, a, lda, af, ldaf, ipiv, equed, r, &
      ...
      character(1), intent(in)    :: fact, trans
      integer,      intent(in)    :: n, nrhs, lda, ldaf, ldb, ldx
      complex(r8),  intent(inout) :: a, af, b            !< a(lda, n), af(ldaf, n), b(ldb, nrhs)
      real(r8),     intent(inout) :: r, c                !< r(n), c(n)
      integer,      intent(inout) :: ipiv                !< ipiv(n)
      real(r8),     intent(out)   :: ferr, berr          !< ferr(nrhs), berr(nrhs)
      real(r8),     intent(out)   :: rcond
      complex(r8),  intent(out)   :: x, work             !< x(ldx, nrhs), work(2*n)
      character(1), intent(inout) :: equed
      real(r8),     intent(out)   :: rwork               !< rwork(2*n)
      integer,      intent(out)   :: info
  ...
  interface
    subroutine dsyevx(jobz, range, uplo, n, a, lda, &
      ...
      real(r8),     intent(in)    :: vl, vu, abstol
      real(r8),     intent(inout) :: a
      real(r8),     intent(out)   :: w, z, work
  ...
  interface lapack_geqrf
    ...
    subroutine zgeqrf(m, n, a, lda, tau, work, lwork, info)
      ...
      integer, intent(in)       :: lda, lwork, m, n
      complex(r8), intent(inout) :: a
      complex(r8), intent(out)   :: tau, work
      integer, intent(out)      :: info
  ...
  interface lapack_sygvx
    subroutine dsygvx(itype, jobz, range, uplo, n, a, lda, b, ldb, vl, vu, il, iu, abstol, &
      ...
      real(r8),             intent(inout) :: a     !< a(:) On entry, the symmetric matrix A. On exit, destroyed
      integer,             intent(in)    :: lda   !< The leading dimension of the array A
      real(r8),             intent(inout) :: b     !< b(:)
  ...
  interface lapack_hegvx
    subroutine zhegvx(itype, jobz, range, uplo, n, a, lda, b, ldb, vl, vu, il, iu, abstol, &
      ...
      complex(r8),          intent(out)   :: work
      integer,             intent(in)    :: lwork
      real(r8),             intent(out)   :: rwork !< rwork(1:7*n)
      integer,             intent(out)   :: iwork !< iwork(1:5*n)
      integer,             intent(out)   :: ifail !< ifail(1:n)
      integer,             intent(out)   :: info
