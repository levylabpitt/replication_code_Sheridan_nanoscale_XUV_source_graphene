...
contains
  ...
  subroutine fft_init(this, nn, dim, type, library, optimize, optimize_parity, mpi_comm, mpi_grp, use_aligned)
    ...
    integer(i8) :: number_points, alloc_size
    ...
    if (.not. (library_ == FFTLIB_NFFT .or. library_ == FFTLIB_PNFFT)) then
      ...
      do idir = 1, dim
        ...
        ! do the multiplication in a integer(i8) to avoid overflow for large grids
      ...
      call messages_write(' (')
      call messages_write(number_points*CNST(8.0), units = unit_megabytes, fmt = '(f9.1)')
      call messages_write(' )')
      if (any(nn(1:fft_dim) /= nn_temp(1:fft_dim))) then
        call messages_new_line()
