...
contains
  ...
  subroutine poisson_isf_init(this, namespace, mesh, cube, all_nodes_comm, init_world)
    ...
    if (.not. mesh%parallel_in_domains) then
      ...
      call build_kernel(cube%rs_n_global(1), cube%rs_n_global(2), cube%rs_n_global(3),   &
        this%cnf(SERIAL)%nfft1, this%cnf(SERIAL)%nfft2, this%cnf(SERIAL)%nfft3, &
        real(cube%spacing(1), 8), order_scaling_function, this%cnf(SERIAL)%kernel)
    end if
    ...
    do i_cnf = 2, N_CNF
      ...
      if (this%cnf(i_cnf)%mpi_grp%rank /= -1 .or. i_cnf /= WORLD) then
        ...
        call par_build_kernel(cube%rs_n_global(1), cube%rs_n_global(2), cube%rs_n_global(3), n1, n2, n3,     &
          ...
          cube%spacing(1), order_scaling_function,                                            &
  ...
  subroutine psolver_kernel(n01, n02, n03, nfft1, nfft2, nfft3, hgrid, karray, rhopot)
    ...
    real(r8), intent(in)    :: hgrid
    real(r8), intent(in)    :: karray(nfft1/2 + 1,nfft2/2 + 1, nfft3/2 + 1)
    real(r8), intent(inout) :: rhopot(n01, n02, n03)

    real(r8), allocatable :: zarray(:,:,:)
    real(r8) :: factor
  ...
  subroutine kernel_application(n1,n2,n3,nd1h,nd2,nd3,nfft1,nfft2,nfft3,zarray,karray,inzee)
    ...
    real(r8), intent(in)    :: karray(1:nfft1/2 + 1, 1:nfft2/2 + 1, 1:nfft3/2 + 1)
    real(r8), intent(inout) :: zarray(1:2, 1:nd1h, 1:nd2, 1:nd3, 1:2)

    real(r8), dimension(:), allocatable :: cos_array,sin_array
    real(r8) :: a,b,c,d,pi2,g1,cp,sp
    real(r8) :: rfe,ife,rfo,ifo,rk,ik,rk2,ik2,re,ro,ie,io,rhk,ihk
  ...
  subroutine zarray_in(n01,n02,n03,nd1,nd2,nd3,density,zarray)
    ...
    real(r8), dimension(n01,n02,n03) :: density
    real(r8), dimension(2,nd1,nd2,nd3) :: zarray
  ...
  subroutine zarray_out(n01, n02, n03, nd1, nd2, nd3, rhopot, zarray, factor)
    ...
    real(r8), intent(out)   :: rhopot(n01,n02,n03)
    real(r8), intent(in)    :: zarray(2*nd1,nd2,nd3)  ! Convert zarray(2,nd1,nd2,nd3) -> zarray(2*nd1,nd2,nd3) to use i1=1,n01
  ...
  subroutine build_kernel(n01,n02,n03,nfft1,nfft2,nfft3,hgrid,itype_scf,karrayout)
    ...
    real(r8), intent(in) :: hgrid
    real(r8), dimension(nfft1/2+1,nfft2/2+1,nfft3/2+1), intent(out) :: karrayout
    ...
    real(r8), parameter :: p0_ref = 1.d0
    real(r8), dimension(N_GAUSS) :: p_gauss,w_gauss

    real(r8), allocatable :: kernel_scf(:), kern_1_scf(:)
    real(r8), allocatable :: x_scf(:), y_scf(:)
    real(r8), allocatable :: karrayhalf(:, :, :)

    real(r8) :: ur_gauss,dr_gauss,acc_gauss,pgauss,kern,a_range
    real(r8) :: factor,factor2,dx,absci,p0gauss,p0_cell
    real(r8) :: a1,a2,a3
  ...
  subroutine karrayhalf_in(n01,n02,n03,n1k,n2k,n3k,nfft1,nfft2,nfft3,nd1,nd2,nd3, kernel,karrayhalf)
    ...
    real(r8), dimension(n1k,n2k,n3k), intent(in) :: kernel
    real(r8), dimension(2,(nd1+1)/2,nd2,nd3), intent(out) :: karrayhalf

    real(r8), dimension(:), allocatable :: karray
  ...
  subroutine kernel_recon(n1k,n2k,n3k,nfft1,nfft2,nfft3,nd1,nd2,nd3,zarray,karray)
    ...
    real(r8), dimension(2,(nd1+1)/2*nd2*nd3), intent(in) :: zarray
    real(r8), dimension(n1k,n2k,n3k), intent(out) :: karray

    real(r8), dimension(:), allocatable :: cos_array,sin_array
    integer :: i1,i2,i3,ind1,ind2,nd1h,n1h,n2h,n3h
    real(r8) :: rfe,ife,rfo,ifo,cp,sp,rk,ik,a,b,c,d,pi2
...
250 if (modulo(nd3,nproc) /= 0) then
  ...
  subroutine par_psolver_kernel(n01, n02, n03, nd1, nd2, nd3, hgrid, kernelLOC, rhopot, iproc, nproc, comm)
    ...
    real(r8), intent(in) :: hgrid
    real(r8), intent(in), dimension(nd1,nd2,nd3/nproc) :: kernelLOC
    real(r8), intent(inout), dimension(n01,n02,n03) :: rhopot
  ...
  subroutine pconvxc_off(m1, m2, m3, n1, n2, n3, nd1, nd2, nd3, md1, md2, md3, iproc, nproc, rhopot, kernelloc, hgrid, comm)
    ...
    real(r8), dimension(nd1,nd2,nd3/nproc), intent(in) :: kernelloc
    real(r8), dimension(m1,m3,m2), intent(inout) :: rhopot
    real(r8), intent(in) :: hgrid
    integer, intent(in) :: comm
    ...
    real(r8) :: scal
    real(r8), dimension(:,:,:), allocatable :: zf, lrhopot(:, :, :)
    ...
    SAFE_ALLOCATE(counts(0:nproc-1))
    SAFE_ALLOCATE(displs(0:nproc-1))
    ...
    do jproc = 0, nproc - 1
      ...
      counts(jproc) = m1*m3*jend
      displs(jproc) = istart*m1*m3
    end do
...
#if defined(HAVE_MPI)
    call MPI_Allgatherv(lrhopot(1, 1, 1), counts(iproc), MPI_DOUBLE_PRECISION, rhopot(1, 1, 1), counts,&
      displs, MPI_DOUBLE_PRECISION, comm, mpi_err)
#endif
    call profiling_out(prof)

    SAFE_DEALLOCATE_A(zf)
    SAFE_DEALLOCATE_A(lrhopot)
    SAFE_DEALLOCATE_A(counts)
...
!!***
  ...
  subroutine enterdensity(rhopot,m1,m2,m3,md1,md2,md3,iproc,nproc,zf)
    ...
    real(r8), dimension(0:md1-1,0:md3-1,0:md2/nproc-1), intent(out) :: zf
    real(r8), dimension(0:m1-1,0:m3-1,0:m2-1), intent(in) :: rhopot
  ...
  subroutine par_build_kernel(n01,n02,n03,nfft1,nfft2,nfft3,n1k,n2k,n3k,hgrid,itype_scf, &
    ...
    real(r8), intent(in)    :: hgrid
    real(r8), intent(out)   :: karrayoutLOC(1:n1k, 1:n2k, 1:n3k/nproc)
    integer, intent(in)    :: comm
    ...
    !(the support of the exponential should be inside [-n_range/2,n_range/2])
    real(r8), parameter :: p0_ref = 1.d0
    real(r8) :: p_gauss(N_GAUSS), w_gauss(N_GAUSS)

    real(r8), dimension(:), allocatable :: kernel_scf,kern_1_scf
    real(r8), dimension(:), allocatable :: x_scf ,y_scf
    real(r8), dimension(:,:,:,:), allocatable :: karrayfour
    real(r8), dimension(:,:,:), allocatable :: karray
...
!!!!END KERNEL CONSTRUCTION
  ...
  subroutine gequad(n_gauss, p_gauss, w_gauss, ur_gauss, dr_gauss, acc_gauss)
    ...
    real(r8), intent(out)   :: p_gauss(:)
    real(r8), intent(out)   :: w_gauss(:)
    real(r8), intent(out)   :: ur_gauss
    real(r8), intent(out)   :: dr_gauss
    real(r8), intent(out)   :: acc_gauss
