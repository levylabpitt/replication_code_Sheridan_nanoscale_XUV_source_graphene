...
subroutine poisson_solve_direct(this, namespace, pot, rho)
  ...
  FLOAT                :: xx1(1:this%der%dim+1), xx2(1:this%der%dim+1), xx3(1:this%der%dim+1), xx4(1:this%der%dim+1)
  FLOAT                :: xx(1:this%der%dim+1), yy(1:this%der%dim+1)

  logical              :: include_diag

  FLOAT                :: xg(this%der%dim)
  integer(i8), allocatable :: ip_v(:)
  ...
  if (this%der%mesh%parallel_in_domains) then
    ...
    do ipg = 1, this%der%mesh%np_global
      ip_v(ipg) = ipg
    end do
    ...
    do ipg = 1, this%der%mesh%np_global
      ...
      tmp(ipg) = dmf_integrate(this%der%mesh, pvec, reduce = .false.)
    end do
      ...
      if (this%der%mesh%use_curvilinear) then
        ...
        do jp = 1, this%der%mesh%np
          ...
          if (ip     /= jp .or. include_diag) then
            aa1 = aa1 + rho(jp)/norm2(xx1(1:dim_eff) - yy(1:dim_eff)) * this%der%mesh%vol_pp(jp)
          end if
          if (ip + 1 /= jp .or. include_diag) then
            aa2 = aa2 + rho(jp)/norm2(xx2(1:dim_eff) - yy(1:dim_eff)) * this%der%mesh%vol_pp(jp)
          end if
          if (ip + 2 /= jp .or. include_diag) then
            aa3 = aa3 + rho(jp)/norm2(xx3(1:dim_eff) - yy(1:dim_eff)) * this%der%mesh%vol_pp(jp)
          end if
          if (ip + 3 /= jp .or. include_diag) then
            aa4 = aa4 + rho(jp)/norm2(xx4(1:dim_eff) - yy(1:dim_eff)) * this%der%mesh%vol_pp(jp)
          end if
        end do
        ...
        do jp = 1, this%der%mesh%np
          yy(1:dim_ele) = this%der%mesh%x(jp, 1:dim_ele)
          if (ip     /= jp .or. include_diag) aa1 = aa1 + rho(jp)/norm2(xx1(1:dim_eff) - yy(1:dim_eff))
          if (ip + 1 /= jp .or. include_diag) aa2 = aa2 + rho(jp)/norm2(xx2(1:dim_eff) - yy(1:dim_eff))
          if (ip + 2 /= jp .or. include_diag) aa3 = aa3 + rho(jp)/norm2(xx3(1:dim_eff) - yy(1:dim_eff))
          if (ip + 3 /= jp .or. include_diag) aa4 = aa4 + rho(jp)/norm2(xx4(1:dim_eff) - yy(1:dim_eff))
        end do
      ...
      if (this%der%mesh%use_curvilinear) then
        ...
        do jp = 1, this%der%mesh%np
          ...
          if (ip == jp .and. .not. include_diag) then
            ...
            aa1 = aa1 + rho(jp)/norm2(xx1(1:dim_eff) - yy(1:dim_eff))
          end if
        end do
      end if
