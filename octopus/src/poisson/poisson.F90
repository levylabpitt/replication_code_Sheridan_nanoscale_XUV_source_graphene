...
contains
  ...
  subroutine poisson_init(this, namespace, space, der, mc, qtot, label, solver, verbose, force_serial, force_cmplx)
    ...
    if (this%method == POISSON_PSOLVER .and. multicomm_have_slaves(mc)) then
      call messages_not_implemented('Task parallelization with LibISF Poisson solver', namespace=namespace)
    end if
    ...
    if (this%method == POISSON_POKE) then
      ...
      call messages_experimental('Poke library', namespace=namespace)
      ASSERT(space%dim == 3)
  ...
  subroutine poisson_slave_work(this, namespace)
    ...
    do while(.not. done)
      ...
      call MPI_Recv(rho(1), this%der%mesh%np, MPI_FLOAT, MPI_ANY_SOURCE, MPI_ANY_TAG, this%intercomm, status, mpi_err)
