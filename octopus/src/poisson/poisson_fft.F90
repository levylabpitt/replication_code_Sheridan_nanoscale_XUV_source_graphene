  ...
  subroutine poisson_fft_get_kernel(namespace, space, cube, coulb, kernel, soft_coulb_param, fullcube)
    ...
    case (3)
      ...
      case (POISSON_FFT_KERNEL_SPH, POISSON_FFT_KERNEL_CORRECTED)
        call poisson_fft_build_3d_0d(namespace,  cube, kernel, coulb)
...
!    end do
  ...
  !-----------------------------------------------------------------
  !> C. A. Rozzi et al., Phys. Rev. B 73, 205119 (2006), Table I
  subroutine poisson_fft_build_3d_0d(namespace, cube, kernel, coulb)
    type(namespace_t),        intent(in)    :: namespace
    type(cube_t),             intent(in)    :: cube
    integer,                  intent(in)    :: kernel
    type(fourier_space_op_t), intent(inout) :: coulb
    ...
    if (kernel /= POISSON_FFT_KERNEL_CORRECTED) then
      ...
      do ix = 1, 3
        temp(1:3) = M_ZERO
        temp(ix) = db(ix)*cube%spacing(ix)/M_TWO
        temp(1:3) = matmul(cube%latt%klattice_primitive(1:3,1:3),temp(1:3))
      ...
      do ly = 1, n2
        ...
        do lz = 1, n3
          ...
          if (abs(modg2) > M_EPSILON) then
            select case (kernel)
            case (POISSON_FFT_KERNEL_SPH)
              fft_Coulb_FS(lx, ly, lz) = M_FOUR*M_PI*poisson_cutoff_3D_0D(sqrt(modg2),r_c)/modg2
            ...
            select case (kernel)
            case (POISSON_FFT_KERNEL_SPH)
              fft_Coulb_FS(lx, ly, lz) = M_FOUR*M_PI*r_c**2/M_TWO
            case (POISSON_FFT_KERNEL_CORRECTED)
              fft_Coulb_FS(lx, ly, lz) = M_ZERO
            end select
          end if
    ...
    do iy = 1, cube%fs_n_global(2)
      ...
      do ix = 1, cube%fs_n_global(1)
        ...
        if (vec*r_c >= x(npoints)) then
          fft_coulb_fs(ix, iy, 1) = y(npoints)
  ...
  subroutine poisson_fft_build_1d_1d(cube, coulb, poisson_soft_coulomb_param)
    ...
    do ix = 1, cube%fs_n_global(1)
      ...
      if (abs(g) > CNST(1e-6)) then
        ...
        fft_coulb_fs(ix, 1, 1) = coulb%singularity * M_TWO
      end if
    end do
