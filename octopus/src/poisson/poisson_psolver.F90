...
contains
  ...
  subroutine poisson_psolver_init(this, namespace, space, cube, mu, qq, force_isolated)
    ...
    alpha = cube%latt%alpha*M_PI/(CNST(180.0))
    beta  = cube%latt%beta*M_PI/(CNST(180.0))
    gamma = cube%latt%gamma*M_PI/(CNST(180.0))
  ...
  !-----------------------------------------------------------------
  subroutine poisson_psolver_reinit(this, space, cube, mu, qq_in)
    type(poisson_psolver_t), intent(inout) :: this
    type(space_t),           intent(in)    :: space
    type(cube_t),            intent(inout) :: cube
    FLOAT,                   intent(in)    :: mu
    FLOAT,                   intent(in)    :: qq_in(:)
    ...
    this%kernel = pkernel_init(cube%mpi_grp%rank, cube%mpi_grp%size, this%inputs,&
      this%geocode,cube%rs_n_global,cube%spacing, &
