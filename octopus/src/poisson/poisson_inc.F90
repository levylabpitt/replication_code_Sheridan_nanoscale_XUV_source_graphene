...
subroutine X(poisson_solve_sm)(this, namespace, sm, pot, rho, all_nodes)
  type(poisson_t),      intent(in)    :: this
  ...
  ASSERT(ubound(pot, dim = 1) == sm%np)
  ASSERT(ubound(rho, dim = 1) == sm%np)
  ...
  case (POISSON_DIRECT_SUM)
    call X(poisson_solve_direct_sm)(this, namespace, sm, pot, rho)
  case (POISSON_ISF)
#ifdef R_TREAL
