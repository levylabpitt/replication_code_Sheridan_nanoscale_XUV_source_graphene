...
contains
  ...
  subroutine initial_state_init(sys, qcstate)
    ...
    if (freeze_orbitals > 0) then
      ...
      call states_elec_freeze_orbitals(psi, sys%namespace, sys%space, sys%gr, sys%mc, sys%kpoints, &
      ...
      call v_ks_calc(sys%ks, sys%namespace, sys%space, sys%hm, psi, sys%ions, sys%ext_partners, calc_eigenval = .true.)
