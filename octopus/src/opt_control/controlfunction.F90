...
contains
  ...
  subroutine controlfunction_mod_init(ext_partners, namespace, dt, max_iter, mode_fixed_fluence)
    ...
    if (.not. varinfo_valid_option('OCTControlFunctionRepresentation', cf_common%representation)) then
    ...
    case (ctr_rt)
      ...
      call messages_info(1, namespace=namespace)
      call messages_experimental('"OCTControlFunctionRepresentation = ctr_rt"', namespace=namespace)
    case default
      write(message(1), '(a)') 'Info: The OCT control functions will be represented in real time.'
      call messages_info(1, namespace=namespace)
      call messages_experimental('"OCTControlFunctionRepresentation = ctr_rt"', namespace=namespace)
    ...
    if (.not. varinfo_valid_option('OCTControlFunctionType', cf_common%mode)) then
      call messages_input_error(namespace, 'OCTControlFunctionType')
    end if
    if (cf_common%representation == ctr_rt .and. (cf_common%mode /= controlfunction_mode_epsilon)) then
      call messages_input_error(namespace, 'OCTControlFunctionType')
    end if
    call messages_print_var_option('OCTControlFunctionType', cf_common%mode, namespace=namespace)

    lasers => list_get_lasers(ext_partners)
    if(.not. associated(lasers)) then
      message(1) = "No lasers found. Cannot proceed."
      call messages_fatal(1, namespace=namespace)
    end if

    ! Check that there are no complex polarization vectors.
    do il = 1, lasers%no_lasers
      pol(:) = laser_polarization(lasers%lasers(il))
      do idir = 1, 3
        if (aimag(pol(idir))**2 > CNST(1.0e-20)) then
          ...
          call messages_fatal(7, namespace=namespace)
        end if
      end do
    end do
    ...
    do il = 1, lasers%no_lasers
      ...
      case (controlfunction_mode_epsilon)
        call laser_to_numerical_all(lasers%lasers(il), dt, max_iter, cf_common%omegamax)
      case default
        call laser_to_numerical(lasers%lasers(il), dt, max_iter, cf_common%omegamax)
      end select
    end do

    ! Fix the carrier frequency
    call messages_obsolete_variable(namespace, 'OCTCarrierFrequency')
    cf_common%w0 = laser_carrier_frequency(lasers%lasers(1))
  ...
  subroutine controlfunction_set(cp, ext_partners)
    type(controlfunction_t), intent(inout) :: cp
    type(partner_list_t),       intent(in) :: ext_partners
  ...
  ! ---------------------------------------------------------
  subroutine controlfunction_to_h(cp, ext_partners)
    ...
    case (controlfunction_mode_epsilon, controlfunction_mode_f)
      do ipar = 1, cp%no_controlfunctions
        call laser_set_f(lasers%lasers(ipar), par%f(ipar))
      end do
    end select
  ...
  ! ---------------------------------------------------------
  subroutine controlfunction_to_h_val(cp, ext_partners, val)
    type(controlfunction_t), intent(in) :: cp
    type(partner_list_t),    intent(in) :: ext_partners
    ...
    do ipar = 1, cp%no_controlfunctions
      call laser_set_f_value(lasers%lasers(ipar), val, tdf(cp%f(ipar), val))
    end do
  ...
  subroutine controlfunction_deltaedeltau(par, dedu)
    ...
    case (ctr_rt)
      dedu = diagonal_matrix(par%dim, M_ONE)
