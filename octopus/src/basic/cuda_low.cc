...
extern "C" void FC_FUNC_(cuda_get_pointer_with_offset, CUDA_GET_POINTER_WITH_OFFSET)
  (CUdeviceptr ** buffer, fint8 * offset, CUdeviceptr ** buffer_offset){
  *buffer_offset = new CUdeviceptr;
  **buffer_offset = (CUdeviceptr)((double*)**buffer + (ptrdiff_t)*offset);
}

extern "C" void FC_FUNC_(cuda_clean_pointer, CUDA_CLEAN_POINTER)(CUdeviceptr ** buffer){
  delete *buffer;
}
