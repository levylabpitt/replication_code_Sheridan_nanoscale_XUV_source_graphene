...
subroutine X(write_binary)(fname, np, ff, ierr, nohead, fendian)
  ...
  ASSERT(product(ubound(ff, kind=i8)) >= np)
...
subroutine X(read_binary2)(fname, np, ff, ierr)
  ...
  iio = 0
  call read_binary(np, 0_i8, c_loc(ff(1,1)), R_TYPE_IOBINARY, ierr, iio, string_f_to_c(fname))
  call io_incr_counters(iio)
...
subroutine X(read_binary3)(fname, np, ff, ierr)
  ...
  iio = 0
  call read_binary(np, 0_i8, c_loc(ff(1,1,1)), R_TYPE_IOBINARY, ierr, iio, string_f_to_c(fname))
  call io_incr_counters(iio)
...
subroutine X(read_binary4)(fname, np, ff, ierr)
  ...
  iio = 0
  call read_binary(np, 0_i8, c_loc(ff(1,1,1,1)), R_TYPE_IOBINARY, ierr, iio, string_f_to_c(fname))
  call io_incr_counters(iio)
...
subroutine X(read_binary5)(fname, np, ff, ierr)
  ...
  iio = 0
  call read_binary(np, 0_i8, c_loc(ff(1,1,1,1,1)), R_TYPE_IOBINARY, ierr, iio, string_f_to_c(fname))
  call io_incr_counters(iio)
