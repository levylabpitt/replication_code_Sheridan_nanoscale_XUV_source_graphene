...
module profiling_oct_m
  ...
  type profile_vars_t
    ...
    integer(i8)              :: alloc_count
    integer(i8)              :: dealloc_count

    integer(i8)              :: memory_limit
    integer(i8)              :: total_memory
    integer(i8)              :: max_memory
    character(len=256)       :: max_memory_location

    integer(i8)              :: large_vars_size(MAX_MEMORY_VARS)
...
contains
  ...
  subroutine profiling_end(namespace)
    ...
    integer(i8) :: io_open_count, io_close_count
    integer(i8) :: io_open_count_red, io_close_count_red
...
#ifdef HAVE_NVTX
    call nvtx_range_push(trim(label), this%index)
#endif
