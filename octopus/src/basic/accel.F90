...
contains
  ...
  subroutine accel_kernel_run_8(kernel, globalsizes, localsizes)
    type(accel_kernel_t), intent(inout) :: kernel
    integer(i8),          intent(in)    :: globalsizes(:)
    integer(i8),          intent(in)    :: localsizes(:)

    integer :: dim
#ifdef HAVE_OPENCL
    integer :: ierr
#endif
    integer(i8) :: gsizes(1:3)
    integer(i8) :: lsizes(1:3)
  ...
  subroutine accel_kernel_run_4(kernel, globalsizes, localsizes)
    ...
    call accel_kernel_run_8(kernel, int(globalsizes, i8), int(localsizes, i8))
  ...
  integer(i8) pure function accel_global_memory_size() result(size)
  ...
  integer(i8) pure function accel_local_memory_size() result(size)
