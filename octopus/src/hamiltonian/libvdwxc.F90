...
contains
  ...
  subroutine libvdwxc_write_info(this, iunit, namespace)
    type(libvdwxc_t),            intent(in) :: this
  ...
  subroutine libvdwxc_set_geometry(this, namespace, space, mesh)
    ...
    ! Here we need to assume that the space is periodic as libvdwxc assumes a
    ! periodic space. This affects the conversion from lattice vectors to the spacing
    cube_space%dim = space%dim
    cube_space%periodic_dim = space%dim
    ...
    call vdwxc_set_unit_cell(this%libvdwxc_ptr, &
      ...
      this%cube%latt%rlattice(3, 3), this%cube%latt%rlattice(2, 3), this%cube%latt%rlattice(1, 3), &
      this%cube%latt%rlattice(3, 2), this%cube%latt%rlattice(2, 2), this%cube%latt%rlattice(1, 2), &
      this%cube%latt%rlattice(3, 1), this%cube%latt%rlattice(2, 1), this%cube%latt%rlattice(1, 1))
  ...
  subroutine libvdwxc_calculate(this, namespace, space, rho, gradrho, dedd, dedgd)
    ...
    real(r8), allocatable :: workbuffer(:)
    real(r8), allocatable :: cube_rho(:,:,:), cube_sigma(:,:,:), cube_dedrho(:,:,:), cube_dedsigma(:,:,:)
    real(r8), dimension(3) :: energy_and_integrals_buffer
    integer :: ii
    ...
    call this%mesh%mpi_grp%allreduce_inplace(energy_and_integrals_buffer(1), 3, MPI_DOUBLE_PRECISION, MPI_SUM)
