...
contains
  ...
  subroutine epot_init(ep, namespace, space, gr, ions, psolver, ispin, xc_family, kpoints)
    ...
    if (ions%ncatoms > 0) then
      ...
      if (ep%classical_pot == CLASSICAL_GAUSSIAN) then
        call messages_experimental("Gaussian smeared classical charges", namespace=namespace)
      ...
      if (ep%classical_pot > 0) then
        ...
        SAFE_ALLOCATE(ep%Vclassical(1:gr%np))
        call epot_generate_classical(ep, namespace, gr, ions)
      end if
    end if
  ...
  subroutine epot_generate(ep, namespace, mesh, ions, st_d)
    ...
    call ion_interaction_calculate(ions%ion_interaction, ions%space, ions%latt, ions%atom, ions%natoms, ions%pos, &
      ions%catom, ions%ncatoms, mesh%box%bounding_box_l, ep%eii, ep%fii)
  ...
  ! ---------------------------------------------------------
  subroutine epot_generate_classical(ep, namespace, mesh, ions)
    type(epot_t),      intent(inout) :: ep
    type(namespace_t), intent(in)    :: namespace
    class(mesh_t),     intent(in)    :: mesh
