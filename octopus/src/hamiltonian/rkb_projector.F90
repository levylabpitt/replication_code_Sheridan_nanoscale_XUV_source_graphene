...
contains
  ...
  subroutine rkb_projector_init(rkb_p, sm, a, l, lm, so_strength)
    ...
    !Build projectors
    do i = 1, 2
      call species_nl_projector(a%species, rkb_p%n_s, sm%rel_x, sm%r, l, lm, i, rkb_p%ket(:, i, 1, 1))
      rkb_p%bra(:, i) = conjg(rkb_p%ket(:, i, 1, 1))
      rkb_p%ket(:, i, 2, 2) = rkb_p%ket(:, i, 1, 1)

      if (lm /= l) then
        call species_nl_projector(a%species, rkb_p%n_s, sm%rel_x, sm%r, l, lm+1, i, rkb_p%ket(:, i, 2, 1))
      else
        rkb_p%ket(:, i, 2, 1) = M_z0
      end if
      if (lm /= -l) then
        call species_nl_projector(a%species, rkb_p%n_s, sm%rel_x, sm%r, l, lm-1, i, rkb_p%ket(:, i, 1, 2))
      else
  ...
  subroutine rkb_project(mesh, sm, rkb_p, psi, ppsi)
    ...
    if (mesh%parallel_in_domains) then
      call mesh%mpi_grp%allreduce(uvpsi(1,1), uvpsi_tmp(1,1), 4, MPI_CMPLX, MPI_SUM)
      uvpsi = uvpsi_tmp
    end if
