    ...
    if (this%coulomb_singularity == SINGULARITY_GENERAL) then
      ...
      case(1)
        ...
        this%FF = this%FF + M_TWO * (M_PI)/kpoints%latt%rcell_volume * length &
          * (M_ONE-log(M_PI / kpoints%latt%rcell_volume * length) + CNST(0.11593151565841244881))
      case(2)
  ...
  contains
    ...
    FLOAT function aux_funct(qq) result(ff)
      ...
      if (this%coulomb_singularity == SINGULARITY_GENERAL) then
        ...
        case(1)
          !The constant is -log(2) + gamma, where gamma is the Euler-Mascheroni constant
          ff =  -log(abs(sin(qq(1)*M_PI))*kpoints%latt%klattice(1,1)/(M_TWO*M_PI)) + CNST(0.11593151565841244881)
        case(3)
