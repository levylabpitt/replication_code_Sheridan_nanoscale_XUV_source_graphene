...
contains
  ...
  function lasers_init(namespace, mesh, space, latt) result(this)
    ...
    type(block_t)     :: blk, blk2
    integer           :: il, ip, jj, ierr, idir, idir2
    ...
    ! Our lasers are currently always 3D and non-periodic
    this%space%dim = 3
    ...
    if (parse_block(namespace, 'TDExternalFields', blk) == 0) then
      ...
      do il = 1, this%no_lasers
        SAFE_ALLOCATE(this%lasers(il)%pol(1:this%space%dim))
        ...
        !%
        !%End
        if (parse_block(namespace, 'MillerIndicesBasis', blk2) == 0) then
        ...
        case (E_FIELD_SCALAR_POTENTIAL)
          ...
          do ip = 1, mesh%np
            call mesh_r(mesh, ip, rr, coords = xx(1:mesh%box%dim))
            xx(mesh%box%dim+1:3) = M_ZERO
            call parse_expression(pot_re, pot_im, this%space%dim, xx, rr, M_ZERO, trim(scalar_pot_expression))
        ...
        case (E_FIELD_MAGNETIC)
          ...
          SAFE_ALLOCATE(this%lasers(il)%a(1:mesh%np_part, 1:this%space%dim))
          this%lasers(il)%a = M_ZERO
          do ip = 1, mesh%np
            xx(1:mesh%box%dim) = mesh%x(ip, :)
            xx(mesh%box%dim+1:3) = M_ZERO
...
!      end if
  ...
  subroutine vlaser_operator_quadratic(laser, der, psi, hpsi)
    ...
    FLOAT :: a_field(3), a_field_prime(3), bb(3), b_prime(3)
