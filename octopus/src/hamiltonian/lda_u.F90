...
contains
  ...
  subroutine lda_u_init(this, namespace, space, level, gr, ions, st, mc, psolver, kpoints, has_phase)
    ...
    if (this%double_couting /= DFT_U_FLL) call messages_experimental("DFTUDoubleCounting = dft_u_amf", namespace=namespace)
    ...
    if (.not. this%basisfromstates) then
      ...
      if (this%level == DFT_U_ACBN0) then
        ...
        if (.not. complex_coulomb_integrals) then
          ...
          if (states_are_real(st)) then
            call dcompute_coulomb_integrals(this, namespace, space, gr, gr%der, psolver)
          else
            call zcompute_coulomb_integrals(this, namespace, space, gr, gr%der, psolver)
          end if
        else
          ASSERT(.not. states_are_real(st))
          write(message(1),'(a)')    'Computing complex Coulomb integrals of the localized basis.'
          call messages_info(1, namespace=namespace)
          call compute_complex_coulomb_integrals(this, gr, gr%der, st, psolver, namespace, space)
        end if
      end if
      ...
      if (states_are_real(st)) then
        call dorbitalbasis_build_empty(this%basis, gr, st%d%kpt, st%d%dim, this%maxnorbs)
      else
        call zorbitalbasis_build_empty(this%basis, gr, st%d%kpt, st%d%dim, this%maxnorbs)
      end if
  ...
  subroutine lda_u_loadbasis(lda_u, namespace, space, st, mesh, mc, ierr)
    ...
    if (accel_is_enabled() .and. st%d%dim == 1) then
      do iorbset = 1, lda_u%norbsets
        os => lda_u%orbsets(iorbset)

        do iorb = 1, os%norbs
          if(states_are_complex(st)) then
            call accel_write_buffer(os%zbuff_orb, mesh%np, os%zorb(:, 1, iorb), &
              offset = (iorb - 1)*os%ldorbs)
          else
            call accel_write_buffer(os%dbuff_orb, mesh%np, os%dorb(:, 1, iorb), &
              offset = (iorb - 1)*os%ldorbs)
          end if
        end do
      end do
