...
contains
  ...
  subroutine species_atom_density(species, namespace, space, latt, pos, mesh, spin_channels, rho)
    ...
    FLOAT :: rr, x, pos_pc(space%dim), nrm, rmax
  ...
  subroutine species_atom_density_grad(species, namespace, space, latt, pos, mesh, spin_channels, drho)
    ...
    FLOAT :: rr, pos_pc(space%dim), range, spline
    type(ps_t), pointer :: ps
    type(lattice_iterator_t) :: latt_iter
  ...
  subroutine species_get_long_range_density(species, namespace, space, latt, pos, mesh, rho, sphere_inout)
    type(species_t),            intent(in)    :: species
    type(namespace_t),          intent(in)    :: namespace
    type(space_t),              intent(in)    :: space
    type(lattice_vectors_t),    intent(in)    :: latt
    ...
    case (SPECIES_PSEUDO, SPECIES_PSPIO)
      ...
      if (present(sphere_inout)) then
        radius_vl  = spline_cutoff_radius(ps%vl, ps%projectors_sphere_threshold) + mesh%spacing(1)
      ...
      if (sphere%np > 0 .and. .not. present(sphere_inout)) then
        do ip = 1, sphere%np
          rho_sphere(ip) = sphere%r(ip)
        end do
        call spline_eval_vec(ps%nlr, sphere%np, rho_sphere)
      else if (sphere%np > 0 .and. present(sphere_inout)) then
        do ip = 1, sphere%np
    ...
    case (SPECIES_FULL_DELTA)
      ...
      if (mesh%parallel_in_domains) then
        ...
        call mesh%mpi_grp%allreduce(local_min, global_min, 1, MPI_2DOUBLE_PRECISION, MPI_MINLOC)
  ...
  subroutine species_get_local(species, namespace, space, latt, pos, mesh, vl)
    ...
    case (SPECIES_USDEF)
      ...
      range = CNST(5.0) * latt%max_length()
      latt_iter = lattice_iterator_t(latt, range)
      vl = M_ZERO
      do icell = 1, latt_iter%n_cells
        pos_pc = pos + latt_iter%get(icell)
    ...
    case (SPECIES_JELLIUM_SLAB)
      a1 = M_TWO *M_PI * species_z(species) / (M_FOUR *mesh%box%bounding_box_l(1) *mesh%box%bounding_box_l(2))
