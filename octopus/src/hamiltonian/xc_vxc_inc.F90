...
subroutine xc_get_vxc(gr, xcs, st, kpoints, psolver, namespace, space, rho, ispin, rcell_volume, &
  ...
  FLOAT,                  intent(in)    :: rcell_volume    !< Lattice unit cell volume.
...
subroutine xc_update_internal_quantities(gr, xcs, st, kpoints, psolver, namespace, space, rho, quantities, ispin, rcell_volume)
  ...
  integer :: families, ip, is, spin_channels
  type(xc_functional_t), pointer :: functl(:)
  logical :: gga, mgga, needs_laplacian
  FLOAT   :: d(2), dtot, dpol

  PUSH_SUB(xc_update_internal_quantities)
  ...
  needs_laplacian = functional_needs_laplacian(functl)
  ...
  case (SPINORS)
    ...
    do ip = 1, gr%np
      ...
      quantities%dens(ip, 1) = max(M_HALF*(dtot + dpol), M_ZERO)
  ...
  POP_SUB(xc_update_internal_quantities)
end subroutine xc_update_internal_quantities

! -----------------------------------------------------
subroutine xc_release_internal_quantities(quantities)
  type(internal_quantities_t), intent(inout)  :: quantities

  PUSH_SUB(xc_release_internal_quantities)

  nullify(quantities%rho)
  SAFE_DEALLOCATE_A(quantities%dens)
  SAFE_DEALLOCATE_A(quantities%gdens)
  SAFE_DEALLOCATE_A(quantities%ldens)
  SAFE_DEALLOCATE_A(quantities%tau)

  POP_SUB(xc_release_internal_quantities)
end subroutine xc_release_internal_quantities

! -----------------------------------------------------
subroutine xc_compute_vxc(der, xcs, st, psolver, namespace, space, quantities, ispin, &
  vxc, ex, ec, deltaxc, vtau, ex_density, ec_density)
  ...
  FLOAT   :: energy(1:2)
  logical :: gga, mgga, mgga_withexc, libvdwxc, needs_laplacian
    ...
    functl_loop: do ixc = FUNC_X, FUNC_C
      ...
      if (family_is_gga(functl(ixc)%family).and.functl(ixc)%family /= XC_FAMILY_LIBVDWXC) then
        ...
        do ib = 1, n_block
          ...
          if (ispin /= UNPOLARIZED) then
            dedgd(ib + ip - 1,:,1) = dedgd(ib + ip - 1,:,1) + l_vsigma(2, ib)*quantities%gdens(ib + ip - 1,:,2)
            dedgd(ib + ip - 1,:,2) = dedgd(ib + ip - 1,:,2)   &
              + M_TWO*l_vsigma(3, ib)*quantities%gdens(ib + ip - 1,:,2) &
              + l_vsigma(2, ib)*quantities%gdens(ib + ip - 1,:,1)
          end if
        end do
      end if
      ...
      if (family_is_mgga(functl(ixc)%family)) then
        ...
        if (bitand(functl(ixc)%flags, XC_FLAGS_HAVE_EXC) /= 0) then
  ...
  if (functl(FUNC_C)%family == XC_FAMILY_LIBVDWXC) then
    ...
    call libvdwxc_calculate(functl(FUNC_C)%libvdwxc, namespace, space, quantities%dens, &
      quantities%gdens, dedd, dedgd)
    if (present(ec)) then
  ...
  if (xcs%xc_density_correction == LR_X) then
    call xc_density_correction_calc(xcs, der, psolver, namespace, space, spin_channels, &
      quantities%dens, vx, dedd, deltaxc = deltaxc)
...
contains
  ...
  subroutine copy_global_to_local(global, local, n_block, spin_channels, ip)
    ...
    PUSH_SUB(xc_compute_vxc.copy_global_to_local)
    ...
    POP_SUB(xc_compute_vxc.copy_global_to_local)
  ...
  subroutine space_loop_init(ip, np, nblock)
    ...
    PUSH_SUB(xc_compute_vxc.space_loop_init)
    ...
    if (gga) then
      ...
      do ib = 1, nblock
        ...
        if (ispin /= UNPOLARIZED) then
          ...
          l_sigma(2, ib) = sum(quantities%gdens(ib + ip - 1, 1:space%dim, 1) &
            * quantities%gdens(ib + ip - 1, 1:space%dim, 2))
          l_sigma(3, ib) = sum(quantities%gdens(ib + ip - 1, 1:space%dim, 2)**2)
        end if
      end do
    end if
    ...
    POP_SUB(xc_compute_vxc.space_loop_init)
  ...
  subroutine lda_init()
    ...
    PUSH_SUB(xc_compute_vxc.lda_init)
  ...
  subroutine lda_end()
    ...
    POP_SUB(xc_compute_vxc.lda_end)
  ...
  subroutine lda_process()
    ...
    PUSH_SUB(xc_compute_vxc.lda_process)
    ...
    POP_SUB(xc_compute_vxc.lda_process)
  ...
  subroutine gga_end()
    PUSH_SUB(xc_compute_vxc.gga_end)

    SAFE_DEALLOCATE_A(dedgd)

    POP_SUB(xc_compute_vxc.gga_end)
  ...
  subroutine gga_process()
    ...
    POP_SUB(xc_compute_vxc.gga_process)
  ...
  subroutine mgga_process()
    ...
    POP_SUB(xc_compute_vxc.mgga_process)
...
pure logical function functional_needs_laplacian(functl)
  ...
  do ixc = 1, 2
    if (bitand(functl(ixc)%flags, XC_FLAGS_NEEDS_LAPLACIAN) /= 0) then
      functional_needs_laplacian = .true.
    end if
  end do
end function functional_needs_laplacian
...
subroutine xc_density_correction_calc(xcs, der, psolver, namespace, space, nspin, density, refvx, vxc, deltaxc)
  ...
  ASSERT(nspin == 1) ! The code below only works for the spin unpolarized case
  ...
  call dpoisson_solve(psolver, namespace, lrvxc, nxc)
