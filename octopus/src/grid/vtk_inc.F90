...
subroutine X(vtk_out_cf_vector)(filename, namespace, fieldname, ierr, cf_in, vector_dim, cube, spacing, unit)
  ...
  call io_binary_write(io_workpath(filename, namespace), vector_dim*np, cfout(:, :, :, :), ierr, &
    nohead = .true., fendian = io_binary_is_little_endian())
...
! ---------------------------------------------------------
!>  Writes a mesh_function in VTK legacy format
!!  for structured grid data in reciprocal space.
!!  The data is defined on a mesh that is topologically
!!  equivalent to a cubic grid. In other words, it must be
!!  laid on a cubic array but the value of each point
!!  coordinate is free in space.
!!
! ---------------------------------------------------------
subroutine X(vtk_out_bz_structured)(filename, namespace, fieldname, ierr, axis, fin, unit, points1BZ, ascii)
  character(len=*),      intent(in) :: filename        !< the file name
  type(namespace_t),     intent(in) :: namespace       !< namespace for directory
  character(len=*),      intent(in) :: fieldname       !< the name of the field
  integer,               intent(out):: ierr            !< error message
  integer,               intent(in) :: axis(3)         !< axis defining the Brillouin zone
  R_TYPE,                intent(in) :: fin(:)          !< the function to be written
  type(unit_t),          intent(in) :: unit            !< unit of data in cf
  FLOAT,                 intent(in) :: points1BZ(:,:)  !< points1BZ(1:3,nik): k-point coordinates
  logical, optional,     intent(in) :: ascii

  integer :: iunit
  integer :: ik, nik
  R_TYPE, allocatable :: fout(:)

  PUSH_SUB(X(vtk_out_bz_structured))

  nik =  product(axis(1:3))
  ...
  if (optional_default(ascii, .false.)) then
    ...
    write(iunit, '(1a)') 'ASCII'
    write(iunit, '(1a)') 'DATASET STRUCTURED_GRID'
    write(iunit, '(1a,3i5)') 'DIMENSIONS', axis(1), axis(2), axis(3)
    write(iunit, '(1a,1i9,1a)') 'POINTS ', nik , ' FLOAT'

    do ik = 1, nik
      write(iunit, '(3f12.6)') points1BZ(1, ik), points1BZ(2, ik), points1BZ(3, ik)
    end do

    write(iunit, '(1a)') ' '
    write(iunit, '(1a,1i9)') 'POINT_DATA ', nik
#ifdef R_TCOMPLEX
    write(iunit, '(3a)') 'SCALARS Re_'//trim(fieldname), ' double 1'
#else
    write(iunit, '(3a)') 'SCALARS ', trim(fieldname), ' double 1'
    ...
    do ik = 1, nik
      write(iunit, '(1f12.6)') R_REAL(units_from_atomic(unit, fin(ik)))
    end do
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    write(iunit, '(1a)') 'DATASET STRUCTURED_GRID'
    write(iunit, '(1a,3i5)') 'DIMENSIONS', axis(1), axis(2), axis(3)
    write(iunit, '(1a,1i9,1a)') 'POINTS ', nik , ' double'
    call io_close(iunit)

    call io_binary_write(io_workpath(filename, namespace), i4_to_i8(nik*3), points1BZ,&
      ierr, nohead = .true., fendian = io_binary_is_little_endian())
    ...
    SAFE_ALLOCATE(fout(1:nik))
    do ik = 1, nik
      fout(ik) = units_from_atomic(unit, fin(ik))
    end do

    call io_binary_write(io_workpath(filename, namespace), i4_to_i8(nik), R_REAL(fout),&
      ierr, nohead = .true., fendian = io_binary_is_little_endian())
    ...
    call io_binary_write(io_workpath(filename, namespace), i4_to_i8(nik), R_AIMAG(fout),&
      ierr, nohead = .true., fendian = io_binary_is_little_endian())
  ...
  POP_SUB(X(vtk_out_bz_structured))

end subroutine X(vtk_out_bz_structured)
