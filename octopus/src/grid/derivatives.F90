...
contains
  ...
  subroutine derivatives_get_stencil_lapl(der, space, coord_system)
    ...
    case (DER_STARGENERAL)
      call stencil_stargeneral_get_arms(der%lapl%stencil, space, coord_system)
  ...
  ! ---------------------------------------------------------
  subroutine derivatives_build(der, namespace, space, mesh, qvector)
    type(derivatives_t),    intent(inout) :: der
    type(namespace_t),      intent(in)    :: namespace
    type(space_t),          intent(in)    :: space
    ...
    case (DER_STAR) ! Laplacian and gradient have different stencils
      do i = 1, der%dim + 1
        ...
        call derivatives_make_discretization(namespace, der%dim, der%mesh, der%masses, polynomials, rhs, 1, der%op(i:i), name)
    ...
    case (DER_CUBE) ! Laplacian and gradient have similar stencils
      ...
      name = "derivatives"
      call derivatives_make_discretization(namespace, der%dim, der%mesh, der%masses, polynomials, rhs, der%dim+1, der%op(:), name)
    ...
    case (DER_STARPLUS)
      ...
      call get_rhs_lapl(rhs(:, 1))
      name = "Laplacian"
      call derivatives_make_discretization(namespace, der%dim, der%mesh, der%masses, polynomials, rhs, 1, &
        der%op(der%dim+1:der%dim+1), name)
      SAFE_DEALLOCATE_A(polynomials)
      SAFE_DEALLOCATE_A(rhs)
    ...
    case (DER_STARGENERAL)
      ...
      SAFE_ALLOCATE(polynomials(1:der%dim, 1:der%op(der%dim+1)%stencil%npoly))
      SAFE_ALLOCATE(rhs(1:der%op(der%dim+1)%stencil%size, 1))
...
!     call exit(1)
  ...
  ! ---------------------------------------------------------
  subroutine derivatives_make_discretization(namespace, dim, mesh, masses, pol, rhs, nderiv, op, name, force_orthogonal)
    type(namespace_t),      intent(in)    :: namespace
    ...
    do p = 1, p_max
      ...
      do i = 1, op(1)%stencil%size
        if (mesh%use_curvilinear) then
          ...
          if (.not. optional_default(force_orthogonal, .false.)) then
            x(1:dim) = mesh%coord_system%to_cartesian(x(1:dim))
          end if
        end if
...
! once we are happy and convinced, remove this comment
  ...
  subroutine derivatives_get_lapl(this, namespace, op, space, name, order)
    ...
    if (.not. this%mesh%coord_system%orthogonal) then
      call stencil_stargeneral_get_arms(op(1)%stencil, space, this%mesh%coord_system)
    ...
    call derivatives_make_discretization(namespace, this%dim, this%mesh, this%masses, &
