...
contains
  ...
  subroutine cube_init(cube, nn, namespace, space, spacing, coord_system, fft_type, fft_library, dont_optimize, nn_out, &
    ...
    FLOAT   :: tp_enlarge_(3), lattice_vectors(3, 3)
    ...
    class is (affine_coordinates_t)
      ...
      my_n(1:space%periodic_dim) = nn(1:space%periodic_dim) + 1
      my_n(space%periodic_dim + 1:space%dim) = nn(space%periodic_dim + 1:space%dim)

      lattice_vectors = M_ZERO
      do idir = 1, space%dim
        do idir2 = 1, space%dim
          lattice_vectors(idir2, idir) = spacing(idir) * (my_n(idir) - 1) * coord_system%basis%vectors(idir2, idir)
        end do
      end do
