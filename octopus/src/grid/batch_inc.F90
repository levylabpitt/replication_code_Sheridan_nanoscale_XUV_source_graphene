...
subroutine X(batch_allocate_unpacked_host)(this)
  ...
  if (this%special_memory) then
    call c_f_pointer(X(allocate_hardware_aware)(int(this%np, i8)*this%dim*this%nst), this%X(ff), &
...
subroutine X(batch_allocate_packed_host)(this)
  ...
  if (this%special_memory) then
    call c_f_pointer(X(allocate_hardware_aware)(int(this%pack_size(1), i8)*this%pack_size(2)), &
      this%X(ff_pack), this%pack_size)
  else
