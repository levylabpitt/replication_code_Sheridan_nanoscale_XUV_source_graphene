...
subroutine X(derivatives_lapl)(der, ff, op_ff, ghost_update, set_bc, factor)
  type(derivatives_t),       intent(in)    :: der
  R_TYPE, contiguous,        intent(inout) :: ff(:)     !< (der%mesh%np_part)
  R_TYPE, contiguous,        intent(out)   :: op_ff(:)  !< (der%mesh%np)
  logical, optional,         intent(in)    :: ghost_update
  logical, optional,         intent(in)    :: set_bc
  FLOAT,   optional,         intent(in)    :: factor
...
subroutine X(derivatives_grad)(der, ff, op_ff, ghost_update, set_bc, to_cartesian)
  type(derivatives_t), intent(in)    :: der
  R_TYPE, contiguous,  intent(inout) :: ff(:)        !< ff(der%mesh%np_part)
  R_TYPE, contiguous,  intent(out)   :: op_ff(:, :)  !< op_ff(der%mesh%np, der%dim)
  logical, optional,   intent(in)    :: ghost_update
  logical, optional,   intent(in)    :: set_bc
...
subroutine X(derivatives_div)(der, ff, op_ff, ghost_update, set_bc, to_cartesian)
  type(derivatives_t),             intent(in)    :: der
  R_TYPE,  contiguous, target,     intent(inout) :: ff(:,:)   !< ff(der%mesh%np_part, der%dim)
  R_TYPE,  contiguous,             intent(out)   :: op_ff(:)  !< op_ff(der%mesh%np)
  logical, optional,               intent(in)    :: ghost_update
  logical, optional,               intent(in)    :: set_bc
  logical, optional,               intent(in)    :: to_cartesian
...
subroutine X(derivatives_test)(this, namespace, repetitions, min_blocksize, max_blocksize)
  ...
  ! test Gradient
  SAFE_ALLOCATE(gradffb(1:this%mesh%box%dim))
  blocksize = min_blocksize
  do
    call X(batch_init)(ffb, 1, 1, blocksize, this%mesh%np_part)
    ...
    write(message(1), '(3a,i3,a,es17.10,a,f8.3)') &
      ...
      ' , Gflops = ',  &
#ifdef R_TREAL
      blocksize*this%mesh%np*M_TWO*this%grad(1)%stencil%size*this%mesh%box%dim/(etime*CNST(1.0e9))
#else
    blocksize*this%mesh%np*M_FOUR*this%grad(1)%stencil%size*this%mesh%box%dim/(etime*CNST(1.0e9))
#endif
    call messages_info(1, namespace=namespace)
...
subroutine X(batch_vector_uvw_to_xyz)(der, coord_system, uvw, xyz, metric)
  ...
  FLOAT,   target, optional,          intent(in)    :: metric(:,:) !< Transpose of the matrix

  class(batch_t), pointer:: xyz_(:)
  class(batch_t), pointer:: uvw_(:)
  integer :: ist, ip, idim1, idim2
  R_TYPE, allocatable :: tmp(:)
  FLOAT, pointer :: metric_(:,:)
...
contains
  subroutine uvw_to_xyz_opencl
    ...
    call accel_write_buffer(matrix_buffer, der%dim**2, transpose(metric_(1:der%dim,1:der%dim)))
...
subroutine X(derivatives_batch_div)(der, ffb, opffb, ghost_update, set_bc, to_cartesian)
  ...
  type(profile_t), save :: batch_divergence_prof

  PUSH_SUB(X(derivatives_batch_div))
  call profiling_in(batch_divergence_prof, TOSTRING(X(BATCH_DIVERGENCE)))
  ...
  call profiling_out(batch_divergence_prof)
