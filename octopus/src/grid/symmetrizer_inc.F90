...
subroutine X(symmetrizer_apply)(this, mesh, field, field_vector, symmfield, symmfield_vector, &
  ...
  if (present(field)) then
    if (mesh%parallel_in_domains) then
      ...
      call par_vec_allgather(mesh%pv, field_global, field)
    else
      field_global => field
    end if
  end if
  ...
  if (present(field_vector)) then
    ...
    if (mesh%parallel_in_domains) then
      ...
      do idir = 1, 3
        call par_vec_allgather(mesh%pv, field_global_vector(:, idir), field_vector(:, idir))
      end do
    else
  ...
  do ip = 1, mesh%np
    ...
    if (present(field)) symmfield(ip) = weight * acc
    if (present(field_vector)) symmfield_vector(ip, 1:3) = weight * acc_vector(1:3)
