  ...
  subroutine batch_copy_data_to(this, np, dest)
    ...
    case (BATCH_PACKED)
      if (np*this%pack_size(1) > huge(0_i4)) then
        ...
        if (dest%type() == TYPE_FLOAT) then
          call blas_copy(int(np*this%pack_size(1), i4), this%dff_pack(1, 1), 1, dest%dff_pack(1, 1), 1)
        else
          call blas_copy(int(np*this%pack_size(1), i4), this%zff_pack(1, 1), 1, dest%zff_pack(1, 1), 1)
        end if
      end if
