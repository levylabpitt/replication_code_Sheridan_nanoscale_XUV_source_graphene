...
contains
  ...
  subroutine pes_flux_init(this, namespace, space, mesh, st, ext_partners, kpoints, abb, save_iter, max_iter)
    ...
    type(block_t)      :: blk
    FLOAT              :: border(space%dim)       ! distance of surface from border
    FLOAT              :: offset(space%dim)       ! offset for border
...
!     end if
  ...
  subroutine pes_flux_reciprocal_mesh_gen(this, namespace, space, st, kpoints, comm, post)
    ...
    FLOAT             :: kmax(space%dim), kmin(space%dim), kact, thetak, phik
    type(block_t)     :: blk

    FLOAT             :: Emin, Emax, DE , kvec(1:3), xx(3)
...
!       this%veca(:, this%itstep) = hm%hm_base%uniform_vector_potential(:)
  ...
  subroutine pes_flux_integrate_cub_tabulate(this, space, mesh, st, kpoints)
    ...
    CMPLX              :: tmp
    FLOAT              :: kpoint(1:3), vec(1:3), lvec(1:3)
...
!       end if
      ...
      do ik = kptst, kptend
        do ist = stst, stend
          do isdim = 1, sdim
            ...
            if (mesh%parallel_in_domains) then
              call mesh%mpi_grp%bcast(s1_act, (lmax + 1) * (2 * lmax + 1) * 3, MPI_CMPLX, itstep - 1)
              call mesh%mpi_grp%bcast(s2_act, (lmax + 1) * (2 * lmax + 1), MPI_CMPLX, itstep - 1)
            end if
  ...
  subroutine pes_flux_getcube(this, mesh, abb, border, offset, fc_ptdens)
    ...
    type(pes_flux_t),             intent(inout) :: this
    type(absorbing_boundaries_t), intent(in)    :: abb
    FLOAT,                        intent(in)    :: border(:)
    FLOAT,                        intent(in)    :: offset(:)
    FLOAT,                        intent(in)    :: fc_ptdens

    integer, allocatable  :: which_surface(:)
    FLOAT                 :: xx(mesh%box%dim), dd, area, dS(mesh%box%dim, 2), factor
    ...
    FLOAT                 :: RSmax(2), RSmin(2), RS(2), dRS(2), weight
    ...
    if (this%surf_interp) then
      ...
      do ip_local = 1, mesh%np
        ...
        case (MASK_ABSORBING)
          in_ab = (abb%mf(ip_local) /= M_ONE)
        case (IMAGINARY_ABSORBING)
          in_ab = (abb%mf(ip_local) /= M_ZERO)
        case default
          ASSERT(.false.)
        end select
