...
contains
  ...
  subroutine propagator_elec_init(gr, namespace, st, tr, have_fields, family_is_mgga_with_exc)
    ...
    if (have_fields) then
      if (tr%method /= PROP_ETRS .and.                &
        ...
        message(1) = "To move the ions or put in a gauge field, use the one of the following propagators:"
        message(2) = "etrs, aetrs, crank-nicolson, rk2, rk4, or exp_mid."
  ...
  subroutine propagator_elec_run_zero_iter(hm, gr, tr)
    ...
    if (family_is_mgga_with_exc(hm%xc)) then
      call potential_interpolation_run_zero_iter(tr%vksold, gr%np, hm%d%nspin, &
  ...
  subroutine propagator_elec_dt(ks, namespace, space, hm, gr, st, tr, time, dt, nt, &
    ...
    if (family_is_mgga_with_exc(hm%xc)) then
      call potential_interpolation_new(tr%vksold, gr%np, st%d%nspin, time, dt, &
        hm%vhxc, vtau = hm%vtau)
    else
      call potential_interpolation_new(tr%vksold, gr%np, st%d%nspin, time, dt, &
        hm%vhxc)
    ...
    case (PROP_QOCT_TDDFT_PROPAGATOR)
      call td_qoct_tddft_propagator(hm, namespace, space, gr, st, tr, time, dt, ions_dyn, ions, ext_partners)
    case (PROP_EXPLICIT_RUNGE_KUTTA4)
  ...
  subroutine propagator_elec_dt_bo(scf, namespace, space, gr, ks, st, hm, ions, ext_partners, mc, outp, iter, dt, ions_dyn, scsteps)
    ...
    call scf_run(scf, namespace, space, mc, gr, ions, ext_partners, st, ks, hm, outp, &
