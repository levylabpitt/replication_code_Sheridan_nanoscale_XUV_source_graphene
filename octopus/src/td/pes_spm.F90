...
contains
  ...
  subroutine pes_spm_init(this, namespace, mesh, st, save_iter)
    ...
    call messages_print_var_option("PES_spm_recipe", this%recipe, namespace=namespace)
    ...
    if (this%omegamax > M_ZERO) then
      ...
      call messages_info(1, namespace=namespace)
      call messages_print_var_value("PES_spm_OmegaMax", this%omegamax, namespace=namespace)
    end if
    ...
    if (this%onfly) then
      ...
      call messages_print_var_value("PES_spm_DeltaOmega", this%delomega, namespace=namespace)
    end if
    ...
    if (this%sphgrid) then
      if (parse_is_defined(namespace, 'PES_spm_Radius')) then
        ...
        call messages_print_var_value("PES_spm_Radius", radius, namespace=namespace)
      else
    ...
    if (.not. this%sphgrid) then
      ...
      if (mdim == 3) call messages_print_var_value("PES_spm_StepsThetaR", this%nstepsthetar, namespace=namespace)
      call messages_print_var_value("PES_spm_StepsPhiR", this%nstepsphir, namespace=namespace)

    end if

    call messages_print_var_value("Number of sample points", this%nspoints, namespace=namespace)
    ...
    if (.not. this%sphgrid) then
      ...
      do isp = 1, this%nspoints
        ...
        this%rcoords(:, isp) = xx(:)
        this%rcoords_nrm(:, isp) = this%rcoords(:, isp) / norm2(this%rcoords(:, isp))
      end do
      ...
      do ith = 0, this%nstepsthetar
        ...
        do iph = 0, this%nstepsphir - 1
          ...
          this%rcoords(:, isp) = radius * this%rcoords_nrm(:, isp)
  ...
  ! ---------------------------------------------------------
  subroutine pes_spm_calc(this, st, mesh, dt, iter, ext_partners)
    type(pes_spm_t),     intent(inout) :: this
    type(states_elec_t), intent(in)    :: st
    type(mesh_t),        intent(in)    :: mesh
    FLOAT,               intent(in)    :: dt
    ...
    if (this%recipe == M_PHASE) then
      call pes_spm_calc_rcphase(this, mesh, iter, dt, ext_partners, itstep)
    end if
    ...
    if (mpi_grp_is_root(mpi_world)) then
      if (.not. this%sphgrid .or. debug%info) then   ! too much output for spherical grid
        do isp = 1, this%nspoints
          ...
          xx(:) = this%rcoords(1:mesh%box%dim, isp)
          write(iunit,'(a1)') '#'
  ...
  subroutine pes_spm_calc_rcphase(this, mesh, iter, dt, ext_partners, ii)
    ...
    if(associated(lasers)) then
      do il = 1, lasers%no_lasers
        call laser_field(lasers%lasers(il), vp, iter*dt)
      end do
      vp = -vp
    end if
    ...
    do isp = 1, this%nspoints
      rdota = dot_product(this%rcoords_nrm(1:mesh%box%dim, isp), vp(1:mesh%box%dim))
      this%dq(isp, ii) = this%dq(isp, iprev) + rdota * dt / P_C
    end do

    this%domega(ii) = this%domega(iprev) + dot_product(vp, vp) / (M_TWO * P_C**M_TWO) * dt
