...
! ---------------------------------------------------------
subroutine pes_mask_output_full_mapM(pesK, file, namespace, space, Lk, ll, how, mesh, pmesh)
  FLOAT,             intent(in) :: pesK(:,:,:)
  ...
  if (bitand(how, OPTION__OUTPUTFORMAT__NETCDF) /= 0) then
    ...
    call dout_cf_netcdf(filename, ierr, cf, cube, space, dk(:), .false., sqrt(units_out%energy)**space%dim, namespace)
    ...
    if (mpi_grp_is_root(st%dom_st_mpi_grp)) then
      ...
      if (st%d%nik == 1) then
        ...
        call io_binary_write(io_workpath(fn, namespace), &
          i4_to_i8(mask%fs_n_global(1))*mask%fs_n_global(2)*mask%fs_n_global(3), pesK, ierr)
...
subroutine pes_mask_write_info(mask, dir, namespace)
  ...
  write(iunit, '(a10,2x,i2)') 'dim', mask%mesh%box%dim
...
subroutine pes_mask_dump(mask, namespace, restart, st, ierr)
  ...
  integer :: itot, ik, ist, idim, ll(3), iunit, err, err2
  integer(i8) :: np
  CMPLX, pointer :: gwf(:,:,:)
  type(profile_t), save :: prof
