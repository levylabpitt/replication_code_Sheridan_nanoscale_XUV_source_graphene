...
contains
  ...
  subroutine states_elec_write_eigenvalues(nst, st, space, kpoints, error, st_start, compact, iunit, namespace)
    ...
    FLOAT :: kpoint(space%dim), max_error
    ...
    do ist = 1, st%nst
      ...
      if (abs(st%occ(ist,tpa_initialk))  <  M_THRESHOLD) then
        ...
        do icoord = 1, space%dim    ! for x,y,z
          ...
          osc_strength = osc_strength + M_TWO/TOFLOAT(space%dim)*transition_energy*abs(osc(icoord))**2
  ...
  subroutine states_elec_write_bandstructure(dir, namespace, nst, st, ions, mesh, kpoints, &
    ...
    FLOAT, optional, allocatable, intent(in)  :: vec_pot_var(:, :) !< (1:box%dim, 1:ns)
