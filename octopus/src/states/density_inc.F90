...
subroutine X(density_accumulate_grad)(space, mesh, st, psib, grad_psib, grad_rho)
  ...
  case (BATCH_DEVICE_PACKED)
    call accel_create_buffer(grad_rho_buff, ACCEL_MEM_WRITE_ONLY, TYPE_FLOAT, mesh%np*space%dim)
    ...
    do idir = 1, space%dim
      ...
      call accel_kernel_run(ker_calc_grad_dens, (/pad(mesh%np, wgsize)/), (/wgsize/))

    end do

    call accel_release_buffer(weights_buff)

    SAFE_ALLOCATE(grad_rho_tmp(1:mesh%np, 1:space%dim))

    call accel_read_buffer(grad_rho_buff, mesh%np*space%dim, grad_rho_tmp)
