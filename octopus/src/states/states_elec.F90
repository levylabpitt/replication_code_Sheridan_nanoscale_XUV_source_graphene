...
module states_elec_oct_m
  ...
  type, extends(states_abst_t) :: states_elec_t
    ...
    FLOAT, allocatable :: current(:, :, :) !<   current(gr%np_part, space%dim, st%d%nspin)
...
contains
  ...
  subroutine states_elec_init(st, namespace, space, valence_charge, kpoints)
    ...
    call messages_print_var_option('SpinComponents', st%d%ispin, namespace=namespace)
...
!!$!!!!ENDOFDEBUG
  ...
  subroutine states_elec_exec_init(st, namespace, mc)
    ...
    call messages_print_var_value('StatesPack', st%d%pack_states, namespace=namespace)
    ...
    call messages_print_var_option('StatesOrthogonalization', st%d%orth_method, namespace=namespace)
  ...
  ! ---------------------------------------------------------
  subroutine states_elec_copy(stout, stin, exclude_wfns, exclude_eigenval)
    type(states_elec_t), target, intent(inout) :: stout
  ...
  subroutine states_elec_distribute_nodes(st, namespace, mc)
    ...
    if ((calc_mode_par_scalapack_compat() .and. .not. st%d%kpt%parallel) .neqv. st%scalapack_compatible) then
  ...
  ! ------------------------------------------------------------
  subroutine states_elec_set_phase(st_d, psi, phase, np, conjugate)
    ...
    PUSH_SUB(states_elec_set_phase)
