...
contains
  ...
  subroutine rdmft_init(rdm, namespace, gr, st, mc, space, fromScratch)
    ...
    if (states_are_complex(st)) then
      call messages_not_implemented("Complex states for RDMFT", namespace=namespace)
    end if
    ...
    if (rdm%do_basis) then
      ...
      rdm%eone_int = M_ZERO
      rdm%twoint = M_ZERO
      rdm%vecnat = diagonal_matrix(rdm%nst, M_ONE)
      rdm%i_index = M_ZERO
      rdm%j_index = M_ZERO
      rdm%k_index = M_ZERO
      rdm%l_index = M_ZERO
      ...
      if (rdm%eigens%additional_terms) call messages_not_implemented("CG Additional Terms with RDMFT", namespace=namespace)
    end if
  ...
  subroutine scf_rdmft(rdm, namespace, space, gr, ions, ext_partners, st, ks, hm, outp, restart_dump)
    type(rdm_t),              intent(inout) :: rdm
    type(namespace_t),        intent(in)    :: namespace
    type(space_t),            intent(in)    :: space
    ...
    call messages_print_stress(msg='RDMFT Calculation', namespace=namespace)
    call messages_print_var_value('RDMBasis', rdm%do_basis, namespace=namespace)
    ...
    if (.not. rdm%do_basis) then
      ...
      call rdm_integrals(rdm, namespace, hm, st, gr)
    ...
    do iter = 1, rdm%max_iter
      ...
      do icount = 1, maxcount 
        if (rdm%do_basis) then
          ...
          call scf_orb_cg(rdm, namespace, space, gr, ions, ext_partners, st, ks, hm, energy)
        end if
      ...
      if ((conv .or. (modulo(iter, outp%restart_write_interval) == 0) .or. iter == rdm%max_iter)) then
        if (rdm%do_basis) then
          ...
          SAFE_ALLOCATE(dpsi2(1:gr%np, 1:st%d%dim))
          do iorb = 1, st%nst
            dpsi = M_ZERO
            do ist = 1, st%nst
              call states_elec_get_state(st, gr, ist, 1, dpsi2)
              do ip = 1, gr%np
                dpsi(ip,1) = dpsi(ip,1) + rdm%vecnat(ist, iorb)*dpsi2(ip,1)
              end do
            end do
            call states_elec_set_state(states_save, gr, iorb, 1, dpsi)
          end do
    ...
    subroutine scf_write_static(dir, fname)
      ...
      if(mpi_grp_is_root(mpi_world)) then
        ...
        if (hm%psolver%is_dressed) then
          ...
          call photon_mode_write_info(hm%psolver%photons, iunit=iunit)
          write(iunit, '(1x)')
        end if
      ...
      if (hm%psolver%is_dressed) then
        call calc_photon_number(space, gr, st, hm%psolver%photons, photon_number_state, ekin_state, epot_state)
        if(mpi_grp_is_root(mpi_world)) then
  ...
  subroutine calc_photon_number(space, gr, st, photons, photon_number_state, ekin_state, epot_state)
    ...
    dim_photon = space%dim

    SAFE_ALLOCATE(psi(1:gr%np_part, 1))
    SAFE_ALLOCATE(psi_q2(1:gr%np))
    SAFE_ALLOCATE(dpsidq(1:gr%np_part))
    SAFE_ALLOCATE(d2psidq2(1:gr%np))
    ...
    do ist = 1, st%nst
      ...
      call dderivatives_partial(gr%der, dpsidq(1:gr%np_part), d2psidq2(:), dim_photon, ghost_update = .true., set_bc = .true.)
      laplace_exp = dmf_dotp(gr, psi(:, 1), d2psidq2(:))
      ekin_state(ist) = -M_HALF*laplace_exp

      ! <phi(ist)|q^2|psi(ist)>= |q|psi(ist)>|^2
      psi_q2(1:gr%np) = psi(1:gr%np, 1) * gr%x(1:gr%np, dim_photon)**2
      q2_exp = dmf_dotp(gr, psi(:, 1), psi_q2(:))
  ...
  subroutine scf_orb_cg(rdm, namespace, space, gr, ions, ext_partners, st, ks, hm, energy)
    ...
    do ik = st%d%kpt%start, st%d%kpt%end
      ...
      call deigensolver_cg(namespace, gr, st, hm, hm%xc, rdm%eigens%pre, rdm%eigens%tolerance, maxiter, &
        rdm%eigens%converged(ik), ik, rdm%eigens%diff(:, ik), rdm%eigens%energy_change_threshold, &
        rdm%eigens%orthogonalize_to_all, rdm%eigens%conjugate_direction, rdm%eigens%additional_terms)
  ...
  subroutine construct_lambda(namespace, hm, st, gr, lambda, rdm)
    ...
    if (.not. rdm%do_basis) then
      SAFE_ALLOCATE(hpsi(1:gr%np,1:st%d%dim))
      SAFE_ALLOCATE(hpsi1(1:gr%np,1:st%d%dim))
      SAFE_ALLOCATE(dpsi(1:gr%np_part ,1:st%d%dim))
      SAFE_ALLOCATE(dpsi1(1:gr%np_part ,1:st%d%dim))

      do iorb = 1, st%nst
        call states_elec_get_state(st, gr, iorb, 1, dpsi)
        call dhamiltonian_elec_apply_single(hm, namespace, gr, dpsi, hpsi, iorb, 1)

        do jorb = iorb, st%nst  
          ! calculate <phi_j|H|phi_i> =lam_ji
          call states_elec_get_state(st, gr, jorb, 1, dpsi1)
          lambda(jorb, iorb) = dmf_dotp(gr, dpsi1(:,1), hpsi(:,1))
          
          ! calculate <phi_i|H|phi_j>=lam_ij
          if (.not. iorb == jorb ) then
            call dhamiltonian_elec_apply_single(hm, namespace, gr, dpsi1, hpsi1, jorb, 1)
            lambda(iorb, jorb) = dmf_dotp(gr, dpsi(:,1), hpsi1(:,1))
          end if
        end do
      end do
  ...
  subroutine rdm_derivatives(rdm, namespace, hm, st, gr, space)
    ...
    if (rdm%do_basis.eqv..false.) then 
      SAFE_ALLOCATE(hpsi(1:gr%np, 1:st%d%dim))
      SAFE_ALLOCATE(rho1(1:gr%np))
      SAFE_ALLOCATE(rho(1:gr%np))
      SAFE_ALLOCATE(dpsi(1:gr%np_part, 1:st%d%dim))
      SAFE_ALLOCATE(dpsi2(1:gr%np, 1:st%d%dim))
      SAFE_ALLOCATE(v_ij(1:gr%np, 1:st%nst, 1:st%nst, 1:st%d%nik, 1:st%d%nik))
      ...
      do ist = 1, st%nst
        call states_elec_get_state(st, gr, ist, 1, dpsi)

        ! calculate one-body energy
        call dhamiltonian_elec_apply_single(hm, namespace, gr, dpsi, hpsi, ist, 1, &
                              terms = TERM_KINETIC + TERM_LOCAL_EXTERNAL + TERM_NON_LOCAL_POTENTIAL)
        rdm%eone(ist) = dmf_dotp(gr, dpsi(:, 1), hpsi(:, 1))
      end do
      ...
      call xst%nullify()
      call dexchange_operator_compute_potentials(hm%exxop, namespace, space, gr, st, xst, hm%kpoints, F_out = v_ij)
      call states_elec_end(xst)

      do ist = 1, st%nst
        call states_elec_get_state(st, gr, ist, 1, dpsi)

        rho1(1:gr%np) = dpsi(1:gr%np, 1)**2

        do jst = ist, st%nst
          rdm%hartree(ist, jst) = dmf_dotp(gr, rho1, v_ij(:,jst, jst, 1, 1))
          rdm%hartree(jst, ist) = rdm%hartree(ist, jst)
          call states_elec_get_state(st, gr, jst, 1, dpsi2)
          rho(1:gr%np) = dpsi2(1:gr%np, 1)*dpsi(1:gr%np, 1)
          rdm%exchange(ist, jst) = dmf_dotp(gr, rho, v_ij(:, ist, jst, 1, 1))
      ...
      do iorb = 1, st%nst
        do jorb =1 , iorb
          rdm%hartree(iorb ,jorb) = M_ZERO
          rdm%exchange(iorb,jorb) = M_ZERO
          do ist =1, st%nst
            do jst =1, st%nst
              dd = rdm%vecnat(ist, iorb)*rdm%vecnat(jst, iorb)
          ...
          rdm%hartree(jorb, iorb) = rdm%hartree(iorb, jorb)
          rdm%exchange(jorb, iorb) = rdm%exchange(iorb, jorb)
        end do
      end do
    end if
