...
contains
  ...
  subroutine lcao_init(this, namespace, space, gr, ions, st)
    ...
    if (space%is_periodic() .and. this%alternative) then
      call messages_experimental("LCAOAlternative in periodic systems", namespace=namespace)
  ...
  subroutine lcao_run(namespace, space, gr, ions, ext_partners, st, ks, hm, st_start, lmm_r)
    ...
    if (.not. present(st_start)) then
      ...
      call v_ks_calc(ks, namespace, space, hm, st, ions, ext_partners, calc_eigenval=.false., &
        calc_current=.false., calc_energy=.false., use_vxc=use_vxc)
  ...
  subroutine lcao_guess_density(this, namespace, st, gr, hm, ions, qtot, ispin, spin_channels, rho)
    ...
    if (spin_channels == 1) then
      ...
      call messages_print_var_option('GuessMagnetDensity', gmd_opt, namespace=namespace)
    end if
    ...
    case (INITRHO_PARAMAGNETIC)
      ...
      do ia = ions%atoms_dist%start, ions%atoms_dist%end
        ...
        rho(1:gr%np, 1:spin_channels) = rho(1:gr%np, 1:spin_channels) + &
          atom_rho(1:gr%np, 1:spin_channels)
      end do
