...
!            orth_mask(ist) = .true. ! projection on unperturbed wfn already removed in Sternheimer eqn
  ...
  if (st%parallel_in_states) then
    call st%mpi_grp%allreduce(eff_mass_inv(1,1,1,1), eff_mass_inv_temp(1,1,1,1), pdim**2 * st%nst * st%d%nik, &
      MPI_FLOAT, MPI_SUM)
    eff_mass_inv(:,:,:,:) = eff_mass_inv_temp(:,:,:,:)
  end if
  if (st%d%kpt%parallel) then
    call st%d%kpt%mpi_grp%allreduce(eff_mass_inv(1,1,1,1), eff_mass_inv_temp(1,1,1,1), pdim**2 * st%nst * st%d%nik, &
      MPI_FLOAT, MPI_SUM)
    eff_mass_inv(:,:,:,:) = eff_mass_inv_temp(:,:,:,:)
  end if
...
! ---------------------------------------------------------
subroutine X(kdotp_add_diagonal)(namespace, space, gr, st, hm, em_pert, kdotp_lr)
  type(namespace_t),        intent(in)    :: namespace
  type(space_t),            intent(in)    :: space
  type(grid_t),             intent(in)    :: gr
  type(states_elec_t),      intent(in)    :: st
