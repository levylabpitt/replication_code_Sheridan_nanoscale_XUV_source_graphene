...
subroutine X(lr_calc_elf)(st, space, gr, kpoints, lr, lr_m)
  ...
  call elf_calc(space, st, gr, kpoints, elf, de)
  ...
  if (states_are_complex(st)) then
    ...
    if (present(lr_m)) then
      ...
      call lr_calc_current(st, space, gr, lr)
    end if
  end if
          ...
          if (present(lr_m)) then
            ...
            do ip = 1, gr%np
              lr%X(dl_de)(ip, is) = lr%X(dl_de)(ip, is) + &
                ...
                rho(ip) * ik_weight * &
                sum(R_CONJ(gpsi(ip, 1:space%dim))*gdl_psi(ip, 1:space%dim) + &
                gpsi(ip, 1:space%dim) * R_CONJ(gdl_psi_m(ip, 1:space%dim)))
            end do
      ...
      do idir1 = 1, ndir
        ...
        do idir2 = 1, ndir
          ...
          do isigma = 1, nsigma
            ...
            call X(inhomog_BE)(namespace, space, gr, st, hm, ions, magn_dir(idir1, 1), magn_dir(idir1, 2), ik, &
...
contains
  subroutine apply_v(add_hartree, add_fxc, nsigma_h, nsigma_in, dir, ist0, ik0, frequency0, &
    ...
    if (add_hartree .or. add_fxc) then
      ...
      frequency0_ = TOFLOAT(frequency0)
#endif
...
subroutine X(inhomog_per_component)(namespace, space, gr, st, hm, ions, idir, ik, psi_k2, psi_out, factor_tot, factor_k, &
  factor_second)
  type(namespace_t),        intent(in)    :: namespace
  type(space_t),            intent(in)    :: space
  type(grid_t),             intent(in)    :: gr
...
! perturbations according to the density matrix formulation.
subroutine X(inhomog_per_component_2nd_order)(namespace, space, gr, st, hm, ions, idir, ik, psi_k2, psi_e, psi_out, factor_tot, &
  factor_k, factor_e)
  type(namespace_t),        intent(in)    :: namespace
  type(space_t),            intent(in)    :: space
  type(grid_t),             intent(in)    :: gr
...
! --------------------------------------------------------------------------
subroutine X(inhomog_BE)(namespace, space, gr, st, hm, ions, idir1, idir2, ik, add_hartree, add_fxc, hvar, psi_e1, psi_e2, &
  psi_ek1, psi_ek2, psi_k1, psi_k2, factor_e1, factor_e2, psi_out)
  type(namespace_t),        intent(in)    :: namespace
  type(space_t),            intent(in)    :: space
  type(grid_t),             intent(in)    :: gr
  type(states_elec_t),      intent(in)    :: st
  ...
  call X(inhomog_per_component_2nd_order)(namespace, space, gr, st, hm, ions, idir1, ik, psi_k2, psi_e2, psi_out, factor_plus, &
    factor_k, factor_e1)
  call X(inhomog_per_component_2nd_order)(namespace, space, gr, st, hm, ions, idir2, ik, psi_k1, psi_e2, psi_out, factor_minus, &
    factor_k, factor_e1)

  call X(inhomog_per_component_2nd_order)(namespace, space, gr, st, hm, ions, idir1, ik, psi_e1, psi_k2, psi_out, factor_plus, &
    factor_e2, factor_k0)
  call X(inhomog_per_component_2nd_order)(namespace, space, gr, st, hm, ions, idir2, ik, psi_e1, psi_k1, psi_out, factor_minus, &
    factor_e2, factor_k0)
...
! --------------------------------------------------------------------------
subroutine X(inhomog_KE)(namespace, space, gr, st, hm, ions, idir, ik, psi_e, psi_out)
  type(namespace_t),        intent(in)    :: namespace
  type(space_t),            intent(in)    :: space
  type(grid_t),             intent(in)    :: gr
  type(states_elec_t),      intent(in)    :: st
...
subroutine X(inhomog_k2)(namespace, space, gr, st, hm, ions, idir1, idir2, ik, psi_k2, psi_out)
  type(namespace_t),        intent(in)    :: namespace
  type(space_t),            intent(in)    :: space
  type(grid_t),             intent(in)    :: gr
  type(states_elec_t),      intent(in)    :: st
...
! --------------------------------------------------------------------------
subroutine X(inhomog_kb)(namespace, gr, space, st, hm, ions, idir, idir1, idir2, ik, psi_b, psi_k1, psi_k2, psi_out)
  type(namespace_t),        intent(in)    :: namespace
  type(space_t),            intent(in)    :: space
  type(grid_t),             intent(in)    :: gr
  type(states_elec_t),      intent(in)    :: st
...
! --------------------------------------------------------------------------
subroutine X(inhomog_k2_tot)(namespace, space, gr, st, hm, ions, idir1, idir2, lr_k1, lr_k2, psi_out)
  type(namespace_t),        intent(in)    :: namespace
  type(space_t),            intent(in)    :: space
  type(grid_t),             intent(in)    :: gr
  type(states_elec_t),      intent(in)    :: st
  ...
  do ik = st%d%kpt%start, st%d%kpt%end
    ik0 = ik - st%d%kpt%start + 1
    call X(inhomog_k2)(namespace, space, gr, st, hm, ions, idir1, idir2, ik, lr_k2(1)%X(dl_psi)(:,:,:, ik), psi_out(:,:,:, ik0, 1))

    if (idir1 == idir2) then
      psi_out(:,:,:, ik0, 1) = M_TWO * psi_out(:,:,:, ik0, 1)
    else
      call X(inhomog_k2)(namespace, space, gr, st, hm, ions, idir2, idir1, ik, lr_k1(1)%X(dl_psi)(:,:,:, ik), psi_out(:,:,:, ik0, 1))
    end if
  end do
