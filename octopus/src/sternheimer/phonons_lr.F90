...
contains
  ...
  subroutine phonons_lr_run_legacy(sys, fromscratch)
    ...
    if (ierr == 0) then
      call states_elec_look_and_load(gs_restart, sys%namespace, sys%space, st, sys%gr, sys%kpoints)
    ...
    if (do_infrared) then
      call born_charges_init(born, sys%namespace, ions, st%val_charge, st%qtot, ndim)
    end if
    ...
    if (states_are_real(st)) then
      call dionic_pert_matrix_elements_2(sys%gr, sys%namespace, sys%space, sys%ions, sys%hm, 1, st, vib, CNST(-1.0), &
        vib%dyn_matrix)
    else
      call zionic_pert_matrix_elements_2(sys%gr, sys%namespace, sys%space, sys%ions, sys%hm, 1, st, vib, CNST(-1.0), &
        vib%dyn_matrix)
    end if
    ...
    do imat = start_mode, vib%num_modes
      ...
      if (.not. fromscratch) then
        ...
        if (ierr == 0) then
          call states_elec_load(restart_load, sys%namespace, sys%space, st, sys%gr, sys%kpoints, ierr, lr = lr(1))
        end if
        if (ierr /= 0) then
    ...
    if (do_infrared) then
      if (sys%space%is_periodic() .and. .not. smear_is_semiconducting(st%smear)) then
        ...
        call born_output_charges(born, ions, sys%namespace, ndim, VIB_MODES_DIR, write_real = .true.)
        call calc_infrared()
      end if
  ...
  contains
    ...
    subroutine build_ionic_dyn_matrix()
      ...
      FLOAT :: term, xi(1:ions%space%dim), xj(1:ions%space%dim), r2
    ...
    subroutine calc_infrared()
      ...
      do iatom = 1, natoms
        do idir = 1, ndim
          ...
          lir(ndim+1) = norm2(lir(1:ndim))/sqrt(TOFLOAT(ndim))
