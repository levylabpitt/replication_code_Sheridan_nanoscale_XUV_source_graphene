...
module em_resp_oct_m
  ...
  type em_resp_t
    ...
    CMPLX   :: alpha(3, 3, 3)                    !< the linear polarizability
    CMPLX   :: alpha_be(3, 3, 3)                 !< the magneto-optical response
    CMPLX   :: alpha0(3, 3, 3)                   !< the linear polarizability without G = G` = 0 term
                                                 !! of the LRC kernel
    CMPLX   :: alpha_be0(3, 3, 3)                !< the magneto-optical response without G = G` = 0
                                                 !! term of the LRC kernel
    CMPLX   :: beta (3, 3, 3)                    !< first hyperpolarizability

    CMPLX   :: chi_para(3, 3)                    !< The paramagnetic part of the susceptibility
    CMPLX   :: chi_dia (3, 3)                    !< The diamagnetic  part of the susceptibility
    CMPLX   :: magn(3)                           !< The orbital magnetization
...
contains
  ...
  subroutine em_resp_run_legacy(sys, fromScratch)
    ...
    FLOAT :: last_omega, frequency, dfrequency_eta
    FLOAT, allocatable :: dl_eig(:,:,:)
    CMPLX :: zfrequency_eta, lrc_coef(sys%space%dim, sys%space%dim)
    ...
    if (use_kdotp) then
      ...
      call restart_init(kdotp_restart, sys%namespace, RESTART_KDOTP, RESTART_TYPE_LOAD, sys%mc, ierr, mesh=sys%gr)
      if (ierr /= 0) then
    ...
    do ifactor = 1, em_vars%nfactor
      call born_charges_init(em_vars%Born_charges(ifactor), sys%namespace, sys%ions, sys%st%val_charge, &
    ...
    if (.not. use_kdotp .or. sys%st%d%nik == 1) em_vars%kpt_output = .false.

    if (em_vars%kpt_output) then
      SAFE_ALLOCATE(em_vars%alpha_k(1:sys%space%dim, 1:sys%space%dim, 1:em_vars%nfactor, 1:sys%st%d%nik))
    end if
    ...
    if (em_vars%calc_magnetooptics) then
      ...
      if (use_kdotp) then
        if (em_vars%kpt_output) then
          SAFE_ALLOCATE(em_vars%alpha_be_k(1:sys%space%dim, 1:sys%space%dim, 1:sys%space%dim, 1:sys%st%d%nik))
        end if
        nfactor_ke = 1
    ...
    subroutine info()
      ...
      type is(perturbation_electric_t)
        if (em_vars%calc_hyperpol) then
          call messages_print_stress(msg='Linear-Response First-Order Hyperpolarizabilities', namespace=sys%namespace)
        else
          call messages_print_stress(msg='Linear-Response Polarizabilities', namespace=sys%namespace)
        end if
      class default
        call messages_print_stress(msg='Magnetic Susceptibilities', namespace=sys%namespace)
      end select
...
#include "undef.F90"
  ...
  subroutine em_resp_output(st, namespace, space, gr, hm, ions, outp, sh, em_vars, iomega, ifactor)
    ...
    logical :: use_kdotp
    CMPLX :: epsilon(space%dim, space%dim)
    ...
    if (sh%has_photons .and. mpi_grp_is_root(mpi_world)) then
    ...
    type is(perturbation_electric_t)
      if ((.not. em_vars%calc_magnetooptics) .or. ifactor == 1) then
        ...
        if (em_vars%calc_Born) then
          call born_output_charges(em_vars%born_charges(ifactor), ions, namespace, space%dim, dirname, &
  ...
  contains
    ...
    subroutine out_polarizability()
      FLOAT :: cross(space%dim, space%dim), crossp(space%dim, space%dim)
      ...
      call output_tensor(TOFLOAT(em_vars%alpha(:, :, ifactor)), space%dim, units_out%polarizability, iunit=iunit)
    ...
    subroutine out_dielectric_constant()
      ...
      call output_tensor(TOFLOAT(epsilon(1:space%dim, 1:space%dim)), space%dim, unit_one, iunit=iunit)
      write(iunit, '(a)')
    ...
    subroutine out_susceptibility()
      ...
      if (.not. use_kdotp) then
        ...
        call output_tensor(TOFLOAT(em_vars%chi_para(:, :)), space%dim, unit_ppm, iunit=iunit)
        write(iunit, '(1x)')

        write(iunit, '(2a)') '# Diamagnetic contribution to the susceptibility tensor [ppm a.u.]'
        call output_tensor(TOFLOAT(em_vars%chi_dia(:, :)), space%dim, unit_ppm, iunit=iunit)
        write(iunit, '(1x)')
      end if

      write(iunit, '(2a)') '# Total susceptibility tensor [ppm a.u.]'
      call output_tensor(TOFLOAT(em_vars%chi_para(:, :) + em_vars%chi_dia(:,:)), &
      ...
      if (.not. use_kdotp) then
        ...
        call output_tensor(TOFLOAT(em_vars%chi_para(:, :)), space%dim, unit_susc_ppm_cgs, iunit=iunit)
        write(iunit, '(1x)')

        write(iunit, '(2a)') '# Diamagnetic contribution to the susceptibility tensor [ppm cgs / mol]'
        call output_tensor(TOFLOAT(em_vars%chi_dia(:, :)), space%dim, unit_susc_ppm_cgs, iunit=iunit)
        write(iunit, '(1x)')
      end if
    ...
    subroutine out_magnetooptics
      integer :: idir, ik
      CMPLX :: epsilon_m(4), diff(4), eps_mk(space%dim)
  ...
  subroutine out_hyperpolarizability(box, beta, freq_factor, converged, dirname, namespace)
    ...
    CMPLX :: bpar(1:box%dim), bper(1:box%dim), bk(1:box%dim)
    CMPLX :: HRS_VV, HRS_HV
