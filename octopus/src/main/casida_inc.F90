...
subroutine X(oscillator_strengths)(cas, mesh, st)
  ...
  if (cas%qcalc) then
    ...
    do ia = 1, cas%n_pairs
      ...
      do ip = 1, mesh%np
        zf(ip) = exp(M_zI*dot_product(cas%qvector(1:mesh%box%dim), mesh%x(ip, 1:mesh%box%dim)))*aimag(zf(ip))*psi_a(ip)
      end do
    ...
    if (cas%avg_order > 0) then
      ...
      do ii = 1, cas%avg_order
        do jj = 1, 2 * cas%avg_order
          ...
          if (mesh%box%dim > 1) qvect(2) = qlen * sin(theta) * sin(phi)
          if (mesh%box%dim > 2) qvect(3) = qlen * cos(phi)
          ...
          do ia = 1, cas%n_pairs
            do ip = 1, mesh%np
              zf(ip) = exp(M_zI*dot_product(qvect(1:mesh%box%dim), mesh%x(ip, 1:mesh%box%dim)))*aimag(zf(ip))*psi_a(ip)
            end do
            zx(ia) = zmf_integrate(mesh, zf, reduce = .false.)
          end do
  ...
  do ia = 1, cas%n
    cas%f(ia) = (M_TWO / mesh%box%dim) * cas%w(ia) * sum((abs(cas%X(tm)(ia, :)))**2)
  end do
  ...
  do ia = 1, cas%n_pairs
    if (loct_isinstringlist(ia, cas%trandens)) then
      ...
      call X(io_function_output)(sys%outp%how(0), CASIDA_DIR, trim(filename), &
        sys%namespace, sys%space, sys%gr, n0I, fn_unit, ierr, ions = sys%ions)
    end if
  end do
...
subroutine X(casida_get_matrix)(cas, namespace, hm, st, ks, mesh, matrix, xc, restart_file, is_forces)
  type(casida_t),           intent(inout) :: cas
    ...
    if (jb <= cas%n_pairs) then
      ...
      if (.not. is_forces_ .and. abs(coeff_vh) > M_EPSILON) then
        ...
        if (hm%theory_level /= INDEPENDENT_PARTICLES) then
          call X(poisson_solve)(hm%psolver, namespace, X(pot), rho_j, all_nodes=.false.)
        end if
      end if
        ...
        do jb = 1, cas%n_pairs
          ...
          if (mpi_grp_is_root(mpi_world)) then
            if (.not. on_this_processor .and. cas%distributed_matrix) then
              ...
              call cas%mpi_grp%recv(write_value, 1, MPI_LOGICAL, src)
              call cas%mpi_grp%recv(save_value, 1, R_MPITYPE, src)
            end if
            if (write_value) call X(write_K_term)(cas, save_value, iunit, ia, jb)
          else
            if (on_this_processor .and. cas%distributed_matrix) then
              call cas%mpi_grp%send(write_value, 1, MPI_LOGICAL, 0)
              call cas%mpi_grp%send(save_value, 1, R_MPITYPE, 0)
            end if
          end if
        end do
      end do
...
contains
  ...
  subroutine load_saved(matrix, is_saved, restart_file)
    ...
    if (mpi_grp_is_root(mpi_world) .or. cas%distributed_matrix) then
      if (iunit > 0) then
        do
          ...
          if (cas%distributed_matrix) then
            call mpi_world%bcast(ii, 1, MPI_INTEGER, 0)
            call mpi_world%bcast(aa, 1, MPI_INTEGER, 0)
            call mpi_world%bcast(ik, 1, MPI_INTEGER, 0)
            call mpi_world%bcast(jj, 1, MPI_INTEGER, 0)
            call mpi_world%bcast(bb, 1, MPI_INTEGER, 0)
            call mpi_world%bcast(jk, 1, MPI_INTEGER, 0)
            call mpi_world%bcast(val, 1, R_MPITYPE, 0)
          end if
    ...
    if (.not. cas%distributed_matrix) then
      call mpi_world%bcast(is_saved(1, 1), cas%n_pairs**2, MPI_LOGICAL, 0)
    end if
...
subroutine X(casida_forces)(cas, sys, mesh, st)
  ...
  do iatom = 1, sys%ions%natoms
    do idir = 1, mesh%box%dim
      ...
      if (cas%type /= CASIDA_EPS_DIFF .and. cas%calc_forces_kernel) then
        ...
        call X(casida_get_matrix)(cas, sys%namespace, sys%hm, st, sys%ks, mesh, cas%X(mat2), lr_fxc, restart_filename, &
          is_forces = .true.)
  ...
  if (cas%calc_forces_scf) then
    call forces_calculate(sys%gr, sys%namespace, sys%ions, sys%hm, sys%ext_partners, st, sys%ks)
    do ia = 1, cas%n_pairs
...
subroutine X(casida_get_lr_hmat1)(cas, sys, iatom, idir, dl_rho, lr_hmat1)
  ...
  call mpi_world%bcast(is_saved(1, 1, 1), cas%nst**2, MPI_LOGICAL, 0)
...
#ifdef HAVE_SCALAPACK
    SAFE_ALLOCATE(eigenvectors(1:cas%nb_rows, 1:cas%nb_cols))
...
subroutine X(casida_write)(cas, sys)
  ...
  integer :: iunit, ia, jb, idim, ia_local, jb_local, index
      ...
      do ia = 1, cas%n
        if (loct_isinstringlist(ia, cas%print_exst) .or. full_printing) then
          ...
          if (cas%calc_forces .and. cas%type /= CASIDA_CASIDA) then
            ...
            call write_xsf_geometry(iunit, sys%ions, sys%gr, forces = cas%forces(:, :, cas%ind(ia)))
            call io_close(iunit)
          end if
        end if
      end do
...
#ifdef HAVE_MPI
  integer :: outfile, mpistatus(MPI_STATUS_SIZE), ierr
#endif
...
#ifdef HAVE_SCALAPACK
    call cas%mpi_grp%allreduce(variable, buffer, 1, R_MPITYPE, MPI_SUM)
    output = buffer
#endif
  end if
