...
contains
  ...
  subroutine casida_run_legacy(sys, fromScratch)
    ...
    if (kpoints_number(sys%kpoints) > 1) then
      ...
      call messages_not_implemented("Casida with k-points", namespace=sys%namespace)
    end if
    if (family_is_mgga_with_exc(sys%hm%xc)) then
      call messages_not_implemented("Casida with MGGA and non-local terms", namespace=sys%namespace)
    end if
    if (sys%hm%lda_u_level /= DFT_U_NONE) then
      call messages_not_implemented("Casida with DFT+U", namespace=sys%namespace)
    end if
    if (sys%hm%theory_level == HARTREE_FOCK) then
      call messages_not_implemented("Casida for Hartree-Fock", namespace=sys%namespace)
    end if
    if (sys%hm%theory_level == GENERALIZED_KOHN_SHAM_DFT) then
      call messages_not_implemented("Casida for generalized Kohn-Sham", namespace=sys%namespace)
    end if
    ...
    if (is_frac_occ) then
      call messages_not_implemented("Casida with partial occupations", namespace=sys%namespace)
    ...
    if (parse_block(sys%namespace, 'CasidaMomentumTransfer', blk) == 0) then
      ...
      call messages_experimental("IXS/EELS transition rate calculation", namespace=sys%namespace)
    ...
    if (cas%calc_forces) then
      call messages_experimental("Excited-state forces calculation", namespace=sys%namespace)
    ...
    if (sys%st%d%ispin /= SPINORS) then
      ...
      if (bitand(theorylevel, CASIDA_TAMM_DANCOFF) /= 0) then
        call messages_experimental("Tamm-Dancoff calculation", namespace=sys%namespace)
      ...
      if (bitand(theorylevel, CASIDA_VARIATIONAL) /= 0) then
        call messages_experimental("CV(2)-DFT calculation", namespace=sys%namespace)
  ...
  subroutine casida_type_init(cas, sys)
    ...
    if (cas%distributed_matrix .and. .not. cas%states_are_real) then
      call messages_not_implemented("Complex wavefunctions with ScaLAPACK layout", namespace=sys%namespace)
    end if
    ...
    if (cas%distributed_matrix .and. .not. cas%parallel_in_eh_pairs) then
      ...
      call messages_info(3, namespace=sys%namespace)
      cas%distributed_matrix = .false.
    end if
    ...
    SAFE_ALLOCATE( cas%index(1:maxval(cas%n_occ), cas%nst - maxval(cas%n_unocc) + 1:cas%nst, 1:cas%nik))
  ...
  subroutine casida_work(sys, cas)
    ...
    case (CASIDA_TAMM_DANCOFF,CASIDA_VARIATIONAL,CASIDA_CASIDA,CASIDA_PETERSILKA)
      if (cas%states_are_real) then
        call dcasida_get_matrix(cas, sys%namespace, sys%hm, st, sys%ks, mesh, cas%dmat, cas%fxc, restart_filename)
        call dcasida_solve(cas, sys)
      else
        call zcasida_get_matrix(cas, sys%namespace, sys%hm, st, sys%ks, mesh, cas%zmat, cas%fxc, restart_filename)
