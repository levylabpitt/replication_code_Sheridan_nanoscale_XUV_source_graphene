...
!    end if
  ...
  ! ---------------------------------------------------------
  subroutine invertks_2part(target_rho, nspin, aux_hm, gr, st, eigensolver, namespace, space, ext_partners, asymptotics)
  ...
  ! ---------------------------------------------------------
  subroutine invertks_iter(target_rho, namespace, space, ext_partners, nspin, aux_hm, gr, &
    st, eigensolver, asymptotics, method)
    type(grid_t),             intent(in)    :: gr
    type(namespace_t),        intent(in)    :: namespace
    type(space_t),            intent(in)    :: space
    type(partner_list_t),     intent(in)    :: ext_partners
...
!                         ks_inversion%aux_st, ks_inversion%eigensolver, namespace, ks_inversion%asymp)
    ...
    case (XC_INV_METHOD_TWO_PARTICLE)
      call invertks_2part(ks_inversion%aux_st%rho, st%d%nspin, ks_inversion%aux_hm, gr, &
        ks_inversion%aux_st, ks_inversion%eigensolver, namespace, space, ext_partners, ks_inversion%asymp)
    case (XC_INV_METHOD_VS_ITER : XC_INV_METHOD_ITER_GODBY)
      call invertks_iter(st%rho, namespace, space, ext_partners, st%d%nspin, ks_inversion%aux_hm, gr, &
