...
subroutine X(preconditioner_apply)(pre, namespace, mesh, hm, a, b, ik, omega)
  ...
  case (PRE_FILTER)
    ...
    call boundaries_set(pre%der%boundaries, mesh, batch_a)
  ...
  case (PRE_POISSON)
    do idim = 1, hm%d%dim
      call X(poisson_solve)(hm%psolver, namespace, b(:, idim), a(:, idim), all_nodes=.false.)
    end do
...
subroutine X(preconditioner_apply_batch)(pre, namespace, mesh, hm, aa, bb, ik, omega)
  ...
  if (pre%which == PRE_FILTER) then
    ...
    call boundaries_set(pre%der%boundaries, mesh, aa)
    select type(aa)
    type is(wfs_elec_t)
      if (allocated(hm%hm_base%phase)) then
        call aa%copy_to(eaa)
