...
contains
  ...
  ! --------------------------------------------------------------------
  subroutine pert_init(this, namespace, pert_type, ions)
    ...
    if (this%pert_type == PERTURBATION_IONIC) then
      SAFE_ALLOCATE(this%ionic%mix1(1:ions%natoms, 1:ions%space%dim))
      SAFE_ALLOCATE(this%ionic%mix2(1:ions%natoms, 1:ions%space%dim))
    end if
