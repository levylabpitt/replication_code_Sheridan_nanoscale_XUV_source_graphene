...
subroutine X(forces_gather)(ions, force)
  ...
  call ions%atoms_dist%mpi_grp%allgatherv(&
    force_local, ions%space%dim*ions%atoms_dist%nlocal, R_MPITYPE, &
    force, recv_count, recv_displ, R_MPITYPE)
...
subroutine X(forces_from_potential)(gr, namespace, space, ions, hm, st, force, force_loc, force_nl, force_u)
  ...
  type(wfs_elec_t) :: psib, grad_psib(space%dim)
  FLOAT :: kweight
  type(profile_t), save :: prof
          ...
          if (st%symmetrize_density .and. hm%kpoints%use_symmetries) then
            ...
            do ii = 1, kpoints_get_num_symmetry_ops(hm%kpoints, ikpoint)
              ...
              do iatom = 1, ions%natoms
                ...
                ratom(:) = symm_op_apply_inv_cart(gr%symm%ops(iop), ions%pos(:, iatom))
                ...
                associate (proj => hm%ep%proj(iatom_symm), boundaries => gr%der%boundaries, dim => st%d%dim)
                  do idir = 1, space%dim
                    force_psi(idir) = - M_TWO * kweight * st%occ(ist, iq) * &
                      R_REAL(X(projector_matrix_element)(proj, boundaries, dim, iq, psi, grad_psi(:, idir, :)))
                  end do
                end associate
    ...
    do ist = st%st_start, st%st_end
      ...
      do idim = 1, st%d%dim
        ...
        if (space%is_periodic() .and. .not. kpoints_point_is_gamma(kpoints, ikpoint)) then
          ...
          kpoint(:) = kpoints%get_point(ikpoint)
