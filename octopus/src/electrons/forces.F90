...
contains
  ...
  subroutine forces_costate_calculate(gr, namespace, ions, hm, psi, chi, ff, qq)
    ...
    FLOAT :: rr, w2r_, w1r_, xx(ions%space%dim), dq, pdot3p, pdot3m, pdot3p2, pdot3m2, dforce1, dforce2
    type(profile_t), save :: forces_prof
    ...
    do iatom = 1, ions%natoms
      do jatom = 1, ions%natoms
        ...
        w2r_ = w2r(ions%atom(iatom)%species, ions%atom(jatom)%species, rr)
        w1r_ = w1r(ions%atom(iatom)%species, ions%atom(jatom)%species, rr)
        do idim = 1, ions%space%dim
          do jdim = 1, ions%space%dim
            ff(iatom, idim) = ff(iatom, idim) + (qq(jatom, jdim) - qq(iatom, jdim)) * w2r_ * (M_ONE/rr**2) * xx(idim) * xx(jdim)
            ff(iatom, idim) = ff(iatom, idim) - (qq(jatom, jdim) - qq(iatom, jdim)) * w1r_ * (M_ONE/rr**3) * xx(idim) * xx(jdim)
            if (jdim == idim) then
              ff(iatom, idim) = ff(iatom, idim) + (qq(jatom, jdim) - qq(iatom, jdim)) * w1r_ * (M_ONE/rr)
            end if
          end do
        end do
      end do
  ...
  contains
    ...
    FLOAT function wr(speca, specb, rr)
      type(species_t), intent(in) :: speca, specb
      FLOAT, intent(in) :: rr
      wr = species_zval(speca) * species_zval(specb) / rr
    end function wr

    FLOAT function w1r(speca, specb, rr)
      type(species_t), intent(in) :: speca, specb
      FLOAT, intent(in) :: rr
      w1r = - species_zval(speca) * species_zval(specb) / rr**2
    end function w1r

    FLOAT function w2r(speca, specb, rr)
      type(species_t), intent(in) :: speca, specb
      FLOAT, intent(in) :: rr
      w2r = M_TWO * species_zval(speca) * species_zval(specb) / rr**3
    end function w2r
  ...
  subroutine forces_calculate(gr, namespace, ions, hm, ext_partners, st, ks, vhxc_old, t, dt)
    ...
    FLOAT :: xx(ions%space%dim), time, global_force(ions%space%dim)
    ...
    do iatom = 1, ions%natoms
      ...
      if (ks%has_photons) then
        ions%tot_force(:, iatom) = ions%tot_force(:, iatom) &
          - P_PROTON_CHARGE*species_zval(ions%atom(iatom)%species)*hm%ep%photon_forces(1:ions%space%dim)
      end if
      ...
      ions%atom(iatom)%f_photons(1:ions%space%dim) = - P_PROTON_CHARGE*species_zval(ions%atom(iatom)%species)* &
        hm%ep%photon_forces(1:ions%space%dim)
    end do
      ...
      do j = 1, lasers%no_lasers
        ...
        case (E_FIELD_ELECTRIC)
          ...
          do iatom = 1, ions%natoms
            ...
            ions%tot_force(:, iatom) = ions%tot_force(:, iatom) + species_zval(ions%atom(iatom)%species)*xx(:)
            ions%atom(iatom)%f_fields(1:ions%space%dim) = ions%atom(iatom)%f_fields(1:ions%space%dim) &
              + species_zval(ions%atom(iatom)%species)*xx(:)
          end do
  ...
  subroutine forces_write_info(iunit, ions, dir, namespace)
    ...
    do iatom = 1, ions%natoms
      write(iunit,'(i4,a10,10es17.8)') iatom, trim(species_label(ions%atom(iatom)%species)), &
    ...
    do iatom = 1, ions%natoms
      write(iunit2,'(i4,a10,30es17.8)') iatom, trim(species_label(ions%atom(iatom)%species)), &
