...
contains
  ...
  subroutine current_calculate(this, namespace, gr, hm, space, st)
    ...
    ASSERT(all(ubound(st%current) == (/gr%np_part, gr%der%dim, st%d%nspin/)))
    ...
    if (st%d%ispin /= SPINORS) then
      ...
      do ik = st%d%kpt%start, st%d%kpt%end
        ispin = st%d%get_spin_index(ik)
        call lalg_axpy(gr%np, gr%der%dim, M_ONE, st%current_kpt(:, :, ik), st%current(:, :, ispin))
      end do
    end if

    if (st%parallel_in_states .or. st%d%kpt%parallel) then
      call comm_allreduce(st%st_kpt_mpi_grp, st%current, dim = (/gr%np, gr%der%dim, st%d%nspin/))
    end if

    if (st%symmetrize_density) then
      do ispin = 1, st%d%nspin
        call dgrid_symmetrize_vector_field(gr, st%current(:, :, ispin), suppress_warning = .true.)
      end do
    end if
