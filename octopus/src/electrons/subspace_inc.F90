...
subroutine X(subspace_diag)(this, namespace, mesh, st, hm, ik, eigenval, diff)
  ...
  call profiling_in(diagon_prof, TOSTRING(X(SUBSPACE_DIAG)))
...
subroutine X(subspace_diag_scalapack)(namespace, mesh, st, hm, ik, eigenval, psi, diff)
  ...
  call profiling_in(prof_diag, TOSTRING(X(SCALAPACK_DIAG)))
...
!(HAVE_ELPA)
  ...
  call profiling_in(prof_gemm2, TOSTRING(X(SCALAPACK_GEMM2)))
...
subroutine X(subspace_diag_hamiltonian)(namespace, mesh, st, hm, ik, hmss)
  ...
  call profiling_in(hamiltonian_elec_prof, TOSTRING(X(SUBSPACE_HAMILTONIAN)))
  ...
  if (mesh%parallel_in_domains) call mesh%allreduce(hmss, dim = (/st%nst, st%nst/))
