...
contains
  ...
  subroutine preconditioner_init(this, namespace, gr, mc, space)
    ...
    call messages_print_var_option('Preconditioner', this%which, namespace=namespace)

    select case (this%which)
    case (PRE_FILTER)
      ! the smoothing is performed uing the same stencil as the Laplacian
      name = "Preconditioner"
      call derivatives_get_lapl(gr%der, namespace, this%op_array, space, name, 1)
      ...
      call messages_print_var_value('PreconditionerFilterFactor', alpha, namespace=namespace)
