...
contains
  ...
  subroutine write_magnetic_moments(mesh, st, ions, boundaries, lmm_r, iunit, namespace)
    ...
    write(message(1), '(a,a,a,f7.3,a)') 'Local Magnetic Moments (sphere radius [', &
    ...
    if (st%d%ispin == SPIN_POLARIZED) then ! collinear spin
      ...
      call messages_info(1, iunit=iunit, namespace=namespace)
      do ia = 1, ions%natoms
        write(message(1),'(i4,a10,f15.6)') ia, trim(species_label(ions%atom(ia)%species)), lmm(3, ia)
        call messages_info(1, iunit=iunit, namespace=namespace)
      end do
    else if (st%d%ispin == SPINORS) then ! non-collinear
      ...
      call messages_info(1, iunit=iunit, namespace=namespace)
      do ia = 1, ions%natoms
        write(message(1),'(i4,a10,9f15.6)') ia, trim(species_label(ions%atom(ia)%species)), lmm(:, ia)
        call messages_info(1, iunit=iunit, namespace=namespace)
      end do
    end if
  ...
  subroutine magnetic_total_magnetization(mesh, st, qq, trans_mag)
    ...
    do ip = 1, mesh%np
      expqr = exp(-M_zI*sum(mesh%x(ip, 1:mesh%box%dim)*qq(1:mesh%box%dim)))
